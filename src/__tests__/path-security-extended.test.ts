...
import { resolveExistingPath, resolvePathWithinBase, isPathWithinBase } from "../path-security"
...
describe("assertSafePath (via resolveExistingPath / resolvePathWithinBase)", () => {
  ...
  test("rejects null byte injection", () => {
    expect(() => resolvePathWithinBase(repoRoot, "valid/path\x00../../etc/passwd", { allowMissing: true })).toThrow(
      "null byte",
    )
  })

  test("rejects URL-encoded traversal (%2e%2e/%2f)", () => {
    expect(() => resolvePathWithinBase(repoRoot, "%2e%2e/%2f", { allowMissing: true })).toThrow("URL-encoded")
  })

  test("rejects URL-encoded dot (%2e%2e)", () => {
    expect(() => resolvePathWithinBase(repoRoot, "%2e%2e", { allowMissing: true })).toThrow("URL-encoded")
  })

  test("rejects URL-encoded backslash (%5c)", () => {
    expect(() => resolvePathWithinBase(repoRoot, "%5c", { allowMissing: true })).toThrow("URL-encoded")
  })

  test("rejects case-insensitive URL encoding (%2E, %2F, %5C)", () => {
    expect(() => resolvePathWithinBase(repoRoot, "%2E%2E", { allowMissing: true })).toThrow("URL-encoded")

    expect(() => resolvePathWithinBase(repoRoot, "%2F", { allowMissing: true })).toThrow("URL-encoded")
  ...
  test("rejects backslash on POSIX", () => {
    ...
    expect(() => resolvePathWithinBase(repoRoot, "..\\..\\etc", { allowMissing: true })).toThrow("backslash")
  })
  ...
  test("double encoding (%252e%252e) — known limitation: not caught by URL-encoded check", () => {
    ...
    expect(() => resolvePathWithinBase(repoRoot, "%252e%252e", { allowMissing: true })).not.toThrow()
  })

  test("allows clean path (src/index.ts)", () => {
    const result = resolvePathWithinBase(repoRoot, "src/index.ts", { allowMissing: true })
    expect(result.resolvedPath).toBe(path.join(fs.realpathSync(repoRoot), "src", "index.ts"))
  })
})
...
describe("resolvePathWithinBase", () => {
  ...
  test("traversal ../../../etc/passwd is blocked", () => {
    expect(() => resolvePathWithinBase(repoRoot, "../../../etc/passwd", { allowMissing: true })).toThrow(
  ...
  test("symlink escape is detected and blocked", () => {
    ...
    symlinkDirectory(outsideDir, path.join(repoRoot, "escape"))
  ...
  test("allowMissing: false (default) for non-existent file throws ENOENT", () => {
    expect(() => resolvePathWithinBase(repoRoot, "src/does-not-exist.ts")).toThrow(/ENOENT/)
  })
})
...
describe("isPathWithinBase", () => {
  ...
  test("subdirectory returns true", () => {
    expect(isPathWithinBase("/home/user/repo", "/home/user/repo/src/index.ts")).toBe(true)
  })

  test("sibling directory returns false", () => {
    expect(isPathWithinBase("/home/user/repo", "/home/user/other-repo/src")).toBe(false)
  })
