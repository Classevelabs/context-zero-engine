...
describe("requirePatchArray", () => {
  ...
  it("returns null for multiple valid patches", () => {
    expect(requirePatchArray([validPatch, { file_path: "src/utils.ts", new_content: "" }])).toBeNull()
  })
  ...
  it("returns error for missing new_content", () => {
    expect(requirePatchArray([{ file_path: "src/index.ts" }])).toMatch(/patches\[0\]\.new_content: required string/)
  })
  ...
  describe("path traversal attacks", () => {
    ...
    it("blocks backslash traversal ..\\..\\etc\\passwd (normalized, then rejected)", () => {
      const result = requirePatchArray([{ file_path: "..\\..\\etc\\passwd", new_content: "" }])
      expect(result).toMatch(/path traversal or absolute path not allowed/)
    })

    it("blocks mixed forward/backslash traversal (normalized, then rejected)", () => {
      const result = requirePatchArray([{ file_path: "..\\../etc/passwd", new_content: "" }])
      expect(result).toMatch(/path traversal or absolute path not allowed/)
    })

    it("accepts a benign Windows-style relative path by normalizing separators", () => {
      const patches = [{ file_path: "src\\db-driver\\result.ts", new_content: "x" }]
      const result = requirePatchArray(patches)
      expect(result).toBeNull()
      expect(patches[0]!.file_path).toBe("src/db-driver/result.ts")
    })

    it("blocks Windows drive-absolute paths like C:\\evil", () => {
      const result = requirePatchArray([{ file_path: "C:\\evil\\x.ts", new_content: "" }])
      expect(result).toMatch(/path traversal or absolute path not allowed/)
    })
...
describe("requireSafePathArray", () => {
  ...
  describe("path traversal attacks", () => {
    ...
    it("accepts a benign Windows-style relative path by normalizing separators", () => {
      const items = ["src\\db-driver\\result.ts"]
      const result = validator(items)
      expect(result).toBeNull()
      expect(items[0]).toBe("src/db-driver/result.ts")
    })
