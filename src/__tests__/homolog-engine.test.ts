...
import type { BehavioralProfile, ContractProfile, HomologCandidate } from "../types"
...
describe("HomologInferenceEngine.findHomologs", () => {
  ...
  test("returns scored homologs sorted by confidence descending", async () => {
    ...
    const cand1 = makeCandidate({
      ...
      body_hash: "SHARED",
      ast_hash: "SHARED", // identical
      signature: "(payment: Payment): Result",
    })
    const cand2 = makeCandidate({
      symbol_version_id: "sv-c2",
      symbol_id: "sym-c2",
      canonical_name: "handleRefund",
      body_hash: "SHARED",
      ast_hash: "different-ast",
    ...
    mockComputeSemanticSimilarity.mockImplementation((_a: string, b: string) =>
      Promise.resolve(b === "sv-c1" ? 0.95 : 0.3),
    )
  ...
  test("uses DEFAULT_HOMOLOG_CONFIDENCE_THRESHOLD when no threshold provided", async () => {
    ...
    expect(DEFAULT_HOMOLOG_CONFIDENCE_THRESHOLD).toBe(0.6)
  })
...
describe("HomologInferenceEngine.persistHomologs", () => {
  ...
  test("persists multiple homologs in a single transaction", async () => {
    const homologs: HomologCandidate[] = [
      ...
      },
      {
        ...
        symbol_name: "b",
        relation_type: "contract_sibling",
        confidence: 0.85,
        evidence: {
          semantic_intent_similarity: 0.6,
          normalized_logic_similarity: 0.3,
          signature_type_similarity: 0.5,
          behavioral_overlap: 0.7,
          contract_overlap: 0.8,
          test_overlap: 0.0,
          history_co_change: 0.0,
          weighted_total: 0.85,
  ...
  test("evidence bundle INSERT includes correct score values", async () => {
    const evidence = {
      ...
      normalized_logic_similarity: 0.9,
      signature_type_similarity: 0.6,
    ...
    expect(bundleParams[2]).toBe(0.9) // structural (normalized_logic)
    expect(bundleParams[3]).toBe(0.45) // behavioral
    expect(bundleParams[4]).toBe(0.3) // contract
    expect(bundleParams[5]).toBe(0.2) // test
  ...
  test("inferred relation INSERT passes correct parameters", async () => {
    const homologs: HomologCandidate[] = [
      {
        symbol_id: "sym-1",
        symbol_version_id: "sv-dest",
        symbol_name: "fn",
        relation_type: "validator_homolog",
        confidence: 0.88,
        evidence: {
          semantic_intent_similarity: 0,
          normalized_logic_similarity: 0,
          signature_type_similarity: 0,
          behavioral_overlap: 0,
          contract_overlap: 0,
          test_overlap: 0,
          history_co_change: 0,
          weighted_total: 0.88,
...
describe("scoreCandidate (via findHomologs)", () => {
  ...
  test("body_hash match yields logic_sim = 1.0", async () => {
    const results = await scoreSingleCandidate({ body_hash: "SHARED" }, { body_hash: "SHARED" }, { semanticSim: 0.5 })
  ...
  test("normalized_ast_hash match yields logic_sim = 0.90", async () => {
    ...
    expect(results.length).toBe(1)
    expect(results[0].evidence.normalized_logic_similarity).toBe(0.9)
  })
    ...
    const results = await scoreSingleCandidate(
      ...
      { body_hash: "SHARED" },
      {
        ...
        contractProfileCandidate: makeContractProfile({
          symbol_version_id: "sv-c1",
          input_contract: "(id: string)",
          output_contract: "Order", // differs
          error_contract: "NotFoundError",
          security_contract: "user", // differs
        }),
      },
    )
    expect(results.length).toBe(1)
...
describe("classifyRelationType (via findHomologs)", () => {
  ...
  test("near_duplicate_logic when logic_sim >= 0.85 (body_hash match)", async () => {
    const relType = await classifySingle({ body_hash: "SHARED" }, { body_hash: "SHARED" })
...
describe("computeNameSimilarity (via semantic fallback)", () => {
  ...
  test("partial overlap gives fractional similarity", async () => {
    ...
    const target = makeTarget({
      body_hash: "SHARED",
      canonical_name: "validateUserInput",
    })
    const cand = makeCandidate({
      symbol_version_id: "sv-c1",
      symbol_id: "sym-c1",
      body_hash: "SHARED",
      canonical_name: "validateOrderInput",
    })
...
describe("Edge cases and constants", () => {
  ...
  test("findHomologs is safe with concurrent calls", async () => {
    ...
    const [r1, r2] = await Promise.all([engine.findHomologs("sv-a", "snap-1"), engine.findHomologs("sv-b", "snap-1")])
    expect(r1).toEqual([])
    expect(r2).toEqual([])
  })
})
