...
describe("DatabaseDriver.query", () => {
  ...
  it("rejects when pool queue exceeds max (default 8x pool size)", async () => {
    // Default limit is maxConnections(20) * 8 = 160 — a legitimate ingest burst
    // (e.g. 100 waiting) must NOT be rejected; genuine runaway load must be.
    mockPool.waitingCount = 100
    mockQuery.mockResolvedValueOnce({ rows: [], rowCount: 0 })
    await expect(db.query("SELECT 1")).resolves.toBeDefined()
    mockPool.waitingCount = 200
  ...
  it("retries on transient error", async () => {
    ...
    mockQuery.mockRejectedValueOnce(transientErr).mockResolvedValueOnce({ rows: [], rowCount: 0 })
