...
import type { PurityClass, TransactionState, SymbolKind, CapsuleMode, ValidationMode } from "../types"
...
describe("Type System Integrity", () => {
  ...
  test("HOMOLOG_WEIGHTS matches blueprint values exactly", () => {
    expect(HOMOLOG_WEIGHTS.semantic_intent_similarity).toBe(0.2)
    expect(HOMOLOG_WEIGHTS.normalized_logic_similarity).toBe(0.2)
    ...
    expect(HOMOLOG_WEIGHTS.test_overlap).toBe(0.1)
