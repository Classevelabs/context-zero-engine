...
describe("handleResolveSymbol", () => {
  ...
  test("returns error for missing query", async () => {
    ...
    ).rejects.toThrow("Missing required string parameter: query")
  })
...
describe("handleFindHomologs", () => {
  ...
  test("defaults confidence_threshold to 0.70", async () => {
    ...
    expect(homologInferenceEngine.findHomologs).toHaveBeenCalledWith(VALID_UUID, VALID_UUID_2, 0.7)
  })
})
...
describe("handleApplyPatch", () => {
  ...
  test("normalizes benign backslash patch file_path instead of rejecting", async () => {
    mockDbQuery.mockResolvedValue({ rows: [{ base_path: "/repo" }] })
    ;(transactionalChangeEngine.applyPatch as jest.Mock).mockResolvedValue(undefined)
    const result = await handleApplyPatch(
      {
    ...
    expect(result.isError).toBeUndefined()
    const applyCall = (transactionalChangeEngine.applyPatch as jest.Mock).mock.calls.at(-1)
    expect(applyCall?.[1]?.[0]?.file_path).toBe("src/main.ts")
  })

  test("returns error for backslash traversal in patch file_path", async () => {
    const result = await handleApplyPatch(
      {
        txn_id: VALID_UUID,
        patches: [{ file_path: "..\\..\\etc\\passwd", new_content: "x" }],
...
describe("handleNativeSymbolSearch", () => {
  ...
  test("returns error when query is missing", async () => {
    ...
    ).rejects.toThrow("Missing required string parameter")
  })
})
