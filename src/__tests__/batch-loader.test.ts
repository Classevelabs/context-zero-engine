...
describe("BatchLoader — chunkedInQuery", () => {
  ...
  test("exactly CHUNK_SIZE (5000) items makes one query", async () => {
    ...
    mockQuery.mockResolvedValueOnce({
      rows: ids.map((id) => ({ behavior_profile_id: `bp-${id}`, symbol_version_id: id })),
      rowCount: 5000,
    })
  ...
  test("5001 items splits into 2 queries (5000 + 1)", async () => {
    ...
    mockQuery
      .mockResolvedValueOnce({
        rows: ids.slice(0, 5000).map((id) => ({ behavior_profile_id: `bp-${id}`, symbol_version_id: id })),
        rowCount: 5000,
      })
      .mockResolvedValueOnce({
  ...
  test("10001 items splits into 3 queries", async () => {
    ...
    mockQuery
      ...
      .mockResolvedValueOnce({
        rows: ids.slice(5000, 10000).map((id) => ({ behavior_profile_id: `bp-${id}`, symbol_version_id: id })),
        rowCount: 5000,
      })
      .mockResolvedValueOnce({
        rows: [{ behavior_profile_id: `bp-${ids[10000]}`, symbol_version_id: ids[10000] }],
        rowCount: 1,
      })
...
describe("BatchLoader — SQL injection prevention", () => {
  test("rejects disallowed table name", async () => {
    ...
    await expect((loader as any).chunkedInQuery("users; DROP TABLE symbols; --", "id", ["1"])).rejects.toThrow(
  ...
  test("rejects disallowed column name", async () => {
    const loader = new BatchLoader()
    await expect((loader as any).chunkedInQuery("behavioral_profiles", "id; DROP TABLE--", ["1"])).rejects.toThrow(
      "disallowed table/column",
    )
  })
