...
describe("EffectEngine", () => {
  ...
  describe("classifyEffectClass", () => {
    ...
    test("escalation order: pure < reader < writer < io < full_side_effect", () => {
      ...
      const fullC = engine.classifyEffectClass([{ kind: "emits", descriptor: "e.e", detail: "", provenance: "direct" }])
  ...
  describe("diffEffects logic", () => {
    test("detects added and removed effects from keyed sets", () => {
      const before: EffectEntry[] = [{ kind: "reads", descriptor: "db.users", detail: "R", provenance: "direct" }]
    ...
    test("detects class escalation from reader to io", () => {
      ...
      const direction =
        ORDER[afterClass]! > ORDER[beforeClass]!
          ? "escalated"
          : ORDER[afterClass]! < ORDER[beforeClass]!
            ? "deescalated"
            : "unchanged"
...
describe("SymbolLineageEngine", () => {
  ...
  describe("computeIdentitySeed", () => {
    ...
    test("same inputs produce identical seed", () => {
      const seed1 = engine.computeIdentitySeed("repo-1", "typescript", "function", "", "foo", "sig", "src")
      const seed2 = engine.computeIdentitySeed("repo-1", "typescript", "function", "", "foo", "sig", "src")
      expect(seed1).toBe(seed2)
    })

    test("different name produces different seed", () => {
      const seed1 = engine.computeIdentitySeed("repo-1", "typescript", "function", "", "foo", "sig", "src")
      const seed2 = engine.computeIdentitySeed("repo-1", "typescript", "function", "", "bar", "sig", "src")
      expect(seed1).not.toBe(seed2)
    })

    test("different language produces different seed", () => {
      const seed1 = engine.computeIdentitySeed("repo-1", "typescript", "function", "", "foo", "sig", "src")
      const seed2 = engine.computeIdentitySeed("repo-1", "python", "function", "", "foo", "sig", "src")
      expect(seed1).not.toBe(seed2)
    })

    test("different kind produces different seed", () => {
      const seed1 = engine.computeIdentitySeed("repo-1", "typescript", "function", "", "User", "sig", "src")
      const seed2 = engine.computeIdentitySeed("repo-1", "typescript", "class", "", "User", "sig", "src")
      expect(seed1).not.toBe(seed2)
    })
  ...
  describe("computeSignatureSimilarity", () => {
    ...
    test("similar signatures have positive score", () => {
      const sim = computeSigSim("(id: string): User", "(userId: string): User")
      expect(sim).toBeGreaterThan(0)
      expect(sim).toBeLessThanOrEqual(1.0)
    })

    test("completely different signatures have lower score", () => {
      const sim = computeSigSim("(id: string): User", "(data: Buffer, options: Config): Promise<void>")
...
describe("TemporalEngine", () => {
  ...
  describe("parseGitLog", () => {
    ...
    test("filters out binary files and lock files", () => {
      const raw =
        "a".repeat(40) +
        "|Dev|dev@test.com|2025-01-10 12:00:00 +0000|Update deps\nsrc/app.ts\npackage-lock.json\nlogo.png\nnode_modules/dep/index.js\n"
      const commits: GitCommit[] = parseGitLog(raw)
  ...
  describe("risk score composite computation", () => {
    ...
    test("regression detection: two bug fixes within 30 days count as regression", () => {
      ...
      for (let i = 1; i < bugFixDates.length; i++) {
        const daysBetween = (bugFixDates[i]!.getTime() - bugFixDates[i - 1]!.getTime()) / (1000 * 60 * 60 * 24)
...
describe("ConceptFamilyEngine", () => {
  ...
  describe("selectExemplarSync", () => {
    ...
    test("returns single member for 1-member family", () => {
      const result = selectExemplar(["sv-1"], [], new Map(), new Map())
  ...
  describe("computeFingerprintsFromCache", () => {
    ...
    test("handles empty contract maps", () => {
      const result = computeFingerprints(["sv-1"], new Map(), new Map())
      const contractFp = JSON.parse(result.contract)
  ...
  describe("detectOutliersFromCache", () => {
    ...
    test("detects purity contradiction between exemplar and member", () => {
      ...
      expect(memberResult?.contradiction_flags).toContain("purity_diverges:pure->side_effecting")
    })
