...
describe("Semantic Integration — TF-IDF Computation", () => {
  ...
  test("computeIDF uses smooth IDF formula", () => {
    const docs = [new Set(["user", "get"]), new Set(["user", "set"]), new Set(["order", "get"])]
...
describe("Semantic Integration — Cosine Similarity", () => {
  test("identical vectors have similarity 1.0", () => {
    ...
    expect(cosineSimilarity(vec, vec)).toBeCloseTo(0.5 * 0.5 + 0.5 * 0.5 + 0.7071 * 0.7071, 3)
  })
  ...
  test("similar code produces higher similarity than different code", () => {
    ...
    const getUserVec = computeTFIDF(computeTF(tokenizeBody("const user = await db.findOne(userId); return user;")), {})
