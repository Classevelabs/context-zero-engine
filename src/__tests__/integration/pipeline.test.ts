...
import type { AdapterExtractionResult, BehaviorHint, ContractHint } from "../../types"
...
describe("Pipeline Integration — TS Adapter Symbol Extraction", () => {
  ...
  beforeAll(async () => {
    result = await extractFromTypeScript([sampleServicePath])
  })
...
describe("Pipeline Integration — Contract Hint Extraction", () => {
  ...
  test("input types are captured", () => {
    const fetchUserHint = result.contract_hints.find((h) => h.symbol_key.includes("fetchUserById"))
  ...
  test("output types are captured", () => {
    const validateHint = result.contract_hints.find((h) => h.symbol_key.includes("validateEmail"))
  ...
  test("createUser captures multiple input types", () => {
    const createHint = result.contract_hints.find((h) => h.symbol_key.includes("createUser"))
...
describe("Pipeline Integration — BehavioralEngine Profile Building", () => {
  ...
  test("builds read_only profile from only read hints", async () => {
    const hints: BehaviorHint[] = [{ symbol_key: "fetchUser", hint_type: "db_read", detail: "raw_query", line: 2 }]
  ...
  test("builds pure profile from no I/O hints", async () => {
    const hints: BehaviorHint[] = [{ symbol_key: "validate", hint_type: "validation", detail: "validate", line: 1 }]
