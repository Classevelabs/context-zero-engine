...
describe("getSymbolDetails", () => {
  ...
  test("throws not found for unknown symbol_version_id", async () => {
    ...
    await expect(getSymbolDetails("sv-nonexistent")).rejects.toThrow("Symbol version not found")
  })
...
describe("getCodebaseOverview", () => {
  ...
  test("returns complete overview structure", async () => {
    ...
    mockQuery.mockResolvedValueOnce({
      rows: [{ path: "src/index.ts" }, { path: "src/utils/helpers.ts" }, { path: "src/styles/main.css" }],
      rowCount: 3,
    })
...
describe("searchCode", () => {
  ...
  test("throws not found when repository does not exist", async () => {
    ...
    await expect(searchCode("bad-repo", "test")).rejects.toThrow("Repository not found")
  })
