...
function makeDirent(
  name: string,
  opts: { isFile?: boolean; isDirectory?: boolean; isSymbolicLink?: boolean } = {},
): any {
  return {
    name,
...
function resetAllMocks() {
  ...
  const allMocks = [
    ...
    mockAddFile,
    mockMergeSymbol,
    mockGetSymbolVersionsForSnapshot,
    mockInsertTestArtifact,
    mockComputeRelationsFromRaw,
    mockExtractBehavioralProfiles,
    mockPropagateTransitive,
    mockExtractContractProfile,
    mockMineInvariantsFromTests,
    mockExtractFromTypeScript,
    mockBatchEmbedSnapshot,
    mockBuildClassHierarchy,
    mockResolveDispatches,
    mockComputeLineage,
    mockComputeEffectSignatures,
    mockPropagateEffectsTransitive,
    mockSynthesizeContracts,
    mockBuildFamilies,
...
describe("Ingestor — discoverFiles (via ingestRepo)", () => {
  ...
  test("discovers .ts files", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValue({ isDirectory: () => true, isFile: () => false, size: 100 })
    mockReaddir.mockResolvedValueOnce([makeDirent("app.ts", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
  ...
  test("skips .git directory", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent(".git", { isDirectory: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
  ...
  test("skips symlinks during file discovery", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent("link.ts", { isFile: true })])
  ...
  test("skips files exceeding MAX_FILE_SIZE (5MB)", async () => {
    ...
    mockReaddir.mockResolvedValueOnce([makeDirent("huge.ts", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
...
describe("Ingestor — Language detection", () => {
  ...
  test(".py maps to python", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent("main.py", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
  ...
  test(".go maps to go", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent("main.go", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
    mockStat.mockResolvedValueOnce({ size: 100 })
  ...
  test(".rs maps to rust", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent("lib.rs", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
    mockStat.mockResolvedValueOnce({ size: 100 })
  ...
  test(".java maps to java", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent("App.java", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
    mockStat.mockResolvedValueOnce({ size: 100 })
  ...
  test(".c, .h, .cu, and .cuh map to cpp (tree-sitter superset)", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([
      makeDirent("main.c", { isFile: true }),
      makeDirent("header.h", { isFile: true }),
      makeDirent("kernel.cu", { isFile: true }),
      makeDirent("kernel.cuh", { isFile: true }),
    ])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
    mockStat
  ...
  test(".rb maps to ruby", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent("app.rb", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
    mockStat.mockResolvedValueOnce({ size: 100 })
  ...
  test(".sh maps to bash", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent("deploy.sh", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
    mockStat.mockResolvedValueOnce({ size: 100 })
...
describe("Ingestor — Cache invalidation", () => {
  ...
  test("clears caches even after failed ingestion", async () => {
    ...
    await expect(ingestor.ingestRepo("/repo", "test-repo", "abc123")).rejects.toThrow()
...
describe("Ingestor — ingestRepo validation", () => {
  test("throws when repoPath does not exist", async () => {
    ...
    await expect(ingestor.ingestRepo("/nonexistent", "test-repo", "abc123")).rejects.toThrow("Invalid repository path")
  })

  test("throws when repoPath is not a directory", async () => {
    mockStat.mockResolvedValueOnce({ isDirectory: () => false })

    await expect(ingestor.ingestRepo("/repo", "test-repo", "abc123")).rejects.toThrow("Invalid repository path")
  })
})
...
describe("Ingestor — Snapshot status transitions", () => {
  ...
  test('sets snapshot status to "partial" when some files fail', async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent("good.ts", { isFile: true }), makeDirent("bad.py", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
    mockStat.mockResolvedValueOnce({ size: 100 }).mockResolvedValueOnce({ size: 100 })
...
describe("Ingestor — Repository and snapshot creation", () => {
  ...
  test('defaults branch to "main" when not provided', async () => {
    ...
    expect(mockCreateSnapshot).toHaveBeenCalledWith(expect.objectContaining({ branch: "main" }))
  })
...
describe("Ingestor — TypeScript extraction", () => {
  ...
  test("counts failed TS extraction against filesFailed", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent("broken.ts", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
    mockStat.mockResolvedValueOnce({ size: 100 })
  ...
  test("finds tsconfig.json when present", async () => {
    ...
    expect(mockExtractFromTypeScript).toHaveBeenCalledWith(expect.any(Array), expect.stringContaining("tsconfig.json"))
  })
})
...
describe("Ingestor — Python extraction", () => {
  test("skips Python extraction when extractor.py is not found", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent("script.py", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
    mockStat.mockResolvedValueOnce({ size: 100 })
    mockReadFile.mockResolvedValue(Buffer.from("x = 1"))
...
describe("Ingestor — hashFile", () => {
  test("produces SHA-256 hex digest of file content", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent("file.ts", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
    mockStat.mockResolvedValueOnce({ size: 100 })
  ...
  test("returns hash of empty string when file read fails", async () => {
    ...
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent("gone.ts", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
    mockStat.mockResolvedValueOnce({ size: 100 })
...
describe("Ingestor — Language set update", () => {
  test("updates repository language_set when files are discovered", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent("app.ts", { isFile: true }), makeDirent("main.py", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
    mockStat.mockResolvedValueOnce({ size: 100 }).mockResolvedValueOnce({ size: 100 })
    mockReadFile.mockResolvedValue(Buffer.from("code"))
...
describe("Ingestor — ingestIncremental", () => {
  ...
  test("a path NEW since the snapshot gets a file row created and is counted in new_files_indexed", async () => {
    ...
    // The invalidation lookup finds NO existing file row — the path is new.
    mockQueryWithClient.mockResolvedValue({ rows: [], rowCount: 0 })
    mockTransaction.mockImplementation(async (cb: any) => {
      const client = { query: jest.fn().mockResolvedValue({ rows: [], rowCount: 0 }) }
      return cb(client)
    ...
    mockAddFile.mockResolvedValue("f-new")
    mockAccess.mockResolvedValue(undefined)
    mockReadFile.mockResolvedValue(Buffer.from("export const brandNew = 1;"))
    mockExtractFromTypeScript.mockReturnValue({
      symbols: [],
      relations: [],
      behavior_hints: [],
    ...
    const result = await ingestor.ingestIncremental("repo-001", "snap-001", ["src/brand-new.ts"])

    // The row was UPSERTED via the same addFile the full ingest uses — the old
    // bare UPDATE silently matched zero rows and the file's symbols vanished.
    expect(mockAddFile).toHaveBeenCalledWith(
      expect.objectContaining({ snapshot_id: "snap-001", path: "src/brand-new.ts", parse_status: "parsed" }),
    )
    expect(result.new_files_indexed).toBe(1)
    expect(mockExtractFromTypeScript).toHaveBeenCalled()
  })
  ...
  test("a KNOWN path refreshes through the same upsert without counting as new", async () => {
    ...
    // The invalidation lookup FINDS the file row — a known, edited file.
    mockTransaction.mockImplementation(async (cb: any) => {
      const client = { query: jest.fn().mockResolvedValue({ rows: [], rowCount: 0 }) }
      mockQueryWithClient
        .mockResolvedValueOnce({ rows: [{ file_id: "f1" }], rowCount: 1 }) // file lookup
        .mockResolvedValue({ rows: [], rowCount: 0 }) // sv lookup + DELETEs
      return cb(client)
    })
    mockAddFile.mockClear()
    mockAddFile.mockResolvedValue("f1")
    mockAccess.mockResolvedValue(undefined)
    mockReadFile.mockResolvedValue(Buffer.from("export const edited = 2;"))
    mockExtractFromTypeScript.mockReturnValue({
      symbols: [],
      relations: [],
      behavior_hints: [],
    ...
    expect(mockAddFile).toHaveBeenCalled()
    expect(result.new_files_indexed).toBe(0)
  })
...
describe("Ingestor — populateTestArtifacts", () => {
  ...
  test("identifies .test.ts files as jest framework", async () => {
    ...
    expect(mockInsertTestArtifact).toHaveBeenCalledWith(expect.objectContaining({ framework: "jest" }))
  })
...
describe("Ingestor — Multiple language handling in single repo", () => {
  test("groups .ts and .js files together for TS adapter", async () => {
    setupLockAcquired()
    mockStat.mockResolvedValueOnce({ isDirectory: () => true })
    mockReaddir.mockResolvedValueOnce([makeDirent("app.ts", { isFile: true }), makeDirent("util.js", { isFile: true })])
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
    mockStat.mockResolvedValueOnce({ size: 100 }).mockResolvedValueOnce({ size: 100 })
    mockReadFile.mockResolvedValue(Buffer.from("code"))
...
describe("Ingestor — SKIP_DIRS coverage", () => {
  test("skips common build/dependency directories", async () => {
    ...
    mockReaddir.mockResolvedValueOnce(skipDirNames.map((name) => makeDirent(name, { isDirectory: true })))
    mockLstat.mockResolvedValue({ isSymbolicLink: () => false })
