...
export class BatchLoader {
  ...
  private async chunkedInQuery<T>(
    ...
    for (let i = 0; i < ids.length; i += CHUNK_SIZE) {
      ...
      try {
        const result = await db.query(`SELECT * FROM ${table} WHERE ${column} IN (${placeholders})`, chunk)
        if (rowValidator) {
  ...
  public async loadSymbolVersionsBySnapshot(snapshotId: string): Promise<SymbolVersionRow[]> {
    ...
    if (rows.length >= BatchLoader.MAX_SNAPSHOT_LOAD) {
      log.warn(
        "loadSymbolVersionsBySnapshot hit safety LIMIT — results truncated. Use loadSymbolVersionsBySnapshotPaginated() for repos with 50K+ symbols.",
        {
          snapshotId,
