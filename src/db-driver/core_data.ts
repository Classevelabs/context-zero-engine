...
export class CoreDataService {
  ...
  public async insertSymbolVersion(input: SymbolVersionInput): Promise<string> {
    ...
    const result = await db.query(
      ...
      [
        ...
        input.snapshot_id,
        input.file_id,
        input.range_start_line,
        input.range_start_col,
        input.range_end_line,
        input.range_end_col,
        input.signature,
        input.ast_hash,
        input.body_hash,
  ...
  public async insertStructuralRelation(input: {
    ...
    const result = await db.query(
      ...
      [
        id,
        input.src_symbol_version_id,
        input.dst_symbol_version_id,
        input.relation_type,
        input.strength ?? 1.0,
        input.source ?? "static_analysis",
        input.confidence,
      ],
    )
  ...
  public async insertInvariant(invariant: {
    ...
    const result = await db.query(
      ...
      [
        id,
        invariant.repo_id,
        invariant.scope_symbol_id,
        invariant.scope_level,
        invariant.expression,
        invariant.source_type,
        invariant.strength,
        invariant.validation_method,
        invariant.last_verified_snapshot_id,
      ],
    )
  ...
  public async insertTestArtifact(artifact: {
    ...
    const result = await db.query(
      ...
      [
        id,
        artifact.symbol_version_id,
        artifact.framework,
        artifact.related_symbols,
        artifact.assertion_summary,
