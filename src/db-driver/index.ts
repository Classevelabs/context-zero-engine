...
class CircuitBreaker {
  ...
  constructor(opts?: { failureThreshold?: number; resetTimeoutMs?: number; halfOpenMaxSuccesses?: number }) {
...
class DatabaseDriver {
  ...
  private constructor() {
    ...
    // Overload protection: reject when the wait queue exceeds this. 8x pool size
    // (not 2x) — legitimate ingest/scoring bursts queue well past 2x, and a
    ...
    const sessionOptions = [
      ...
      `-c idle_in_transaction_session_timeout=${idleInTxnTimeoutMs}ms`,
    ].join(" ")
    this.pool = new Pool({
      ...connConfigWithOptions,
      max: maxConnections,
      ...
      options: [existingOptions, sessionOptions].filter(Boolean).join(" "),
    })
   ...
   */
  public async transaction<T>(callback: (client: PoolClient) => Promise<T>, isolation?: IsolationLevel): Promise<T> {
    this.ensureOpen()
    this.circuit.check()
  ...
  public async bulkInsert(
    ...
    for (let i = 0; i < rows.length; i++) {
      if (rows[i]!.length !== colCount) {
        throw new Error(`bulkInsert: row ${i} has ${rows[i]!.length} values, expected ${colCount}`)
      }
    }
    ...
    const runStatement = async (chunk: unknown[][], client: PoolClient | DatabaseDriver): Promise<number> => {
      ...
      const result =
        client instanceof DatabaseDriver ? await client.query(sql, params) : await client.query(sql, params)
  ...
  public async healthCheck(): Promise<{
    ...
    try {
      ...
      let pgTrgm = false
      try {
        const extResult = await this.pool.query("SELECT extname FROM pg_extension WHERE extname = 'pg_trgm'")
