...
const VALID_SSL_MODES: ReadonlySet<string> = new Set<SslMode>(["disable", "require", "verify-ca", "verify-full"])
...
function readBoolEnv(keys: string[], fallback = false): boolean {
  for (const key of keys) {
    const value = process.env[key]
    if (typeof value !== "string" || value.trim().length === 0) continue
 ...
 * enforced by default. Container/private-network deployments may explicitly set
 * DB_SSL_ALLOW_INSECURE_PRIVATE_NETWORK=true to opt out.
 */
function buildSslConfig(host: string): false | tls.ConnectionOptions {
  const rawMode = (process.env["DB_SSL_MODE"] || "").trim().toLowerCase()
  const mode: SslMode = rawMode && VALID_SSL_MODES.has(rawMode) ? (rawMode as SslMode) : "disable"
  ...
  const allowInsecurePrivateNetwork = readBoolEnv(["DB_SSL_ALLOW_INSECURE_PRIVATE_NETWORK"], false)

  // --- Production safety checks ---------------------------------------------------
  if (isProduction()) {
    if (mode === "disable" && isRemoteHost && !allowInsecurePrivateNetwork) {
      throw new Error(
        "Refusing to connect to a remote database without SSL in production. " +
          `DB_HOST="${host}" is not localhost. ` +
          'Set DB_SSL_MODE to "verify-ca" or "verify-full", or set ' +
          "DB_SSL_ALLOW_INSECURE_PRIVATE_NETWORK=true only for a trusted private network.",
      )
    }
    ...
    if (mode === "disable") {
      log.warn(
        allowInsecurePrivateNetwork && isRemoteHost
          ? "DB_SSL_MODE=disable in production — database traffic is unencrypted on a configured private network"
          : "DB_SSL_MODE=disable in production — database traffic is unencrypted (localhost only)",
      )
    }
  }
  ...
  if (mode === "verify-ca" || mode === "verify-full") {
    ...
    if (!caPath) {
      throw new Error(
        `DB_SSL_MODE="${mode}" requires DB_SSL_CA to be set to the path ` + "of a PEM-encoded CA certificate file.",
      )
    }

    if (!fs.existsSync(caPath)) {
      throw new Error(
        `CA certificate file not found: "${caPath}". ` + "Ensure DB_SSL_CA points to an existing PEM file.",
      )
    }
