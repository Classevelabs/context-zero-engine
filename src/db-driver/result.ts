...
export function requireFirstRow(result: Pick<QueryResult, "rows">, context: string): QueryRow {
...
export function validateStructuralRelation(row: QueryRow): StructuralRelation | null {
  ...
  return {
    ...
    relation_type: (typeof row["relation_type"] === "string"
      ? row["relation_type"]
      : "calls") as StructuralRelationType,
    strength: typeof row["strength"] === "number" ? row["strength"] : 1.0,
    source: (typeof row["source"] === "string" ? row["source"] : "static_analysis") as
      | "static_analysis"
      | "runtime_trace"
      | "heuristic"
      | "manual",
    ...
    provenance:
      typeof row["provenance"] === "string" ? (row["provenance"] as StructuralRelation["provenance"]) : undefined,
  }
}
 ...
 */
export function validateRows<T>(rows: unknown[], validator: (row: QueryRow) => T | null, context: string): T[] {
  const results: T[] = []
  let invalidCount = 0
