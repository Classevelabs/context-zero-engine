...
const ALLOW_ANY_CORS_ORIGIN = CORS_ORIGINS.includes("*")

app.use((req: Request, res: Response, next: NextFunction) => {
  const origin = req.headers.origin
  if (origin && (ALLOW_ANY_CORS_ORIGIN || CORS_ORIGINS.includes(origin))) {
    res.setHeader("Access-Control-Allow-Origin", ALLOW_ANY_CORS_ORIGIN ? "*" : origin)
...
function safeHandler(handler: (req: Request, res: Response) => Promise<void>): (req: Request, res: Response) => void {
...
app.post(
  ...
  safeHandler(async (req, res) => {
    ...
    const report = await blastRadiusEngine.computeBlastRadius(snapshot_id, symbol_version_ids, depth)
...
app.post(
  ...
  safeHandler(async (req, res) => {
    ...
    const branchName = (branch as string | undefined) || (repo.default_branch as string | undefined) || "main"
...
app.post(
  ...
  safeHandler(async (req, res) => {
    const { source_symbol_version_id, snapshot_id, confidence_threshold = 0.7 } = req.body
    ...
    symbol_version_ids: (value: unknown): string | null => {
      ...
      for (let i = 0; i < value.length; i++) {
        if (
          ...
          !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(value[i] as string)
        ) {
  ...
  safeHandler(async (req, res) => {
    ...
    if (ids.length > 0) {
      ...
      if (svResult.rows.length === 0) {
        res.status(404).json({ error: "No symbol versions found" })
        return
      }
...
app.post(
  ...
  safeHandler(async (req, res) => {
    ...
    if (
      !rawTracePack.source ||
      typeof rawTracePack.source !== "string" ||
      !validSources.includes(rawTracePack.source)
    ) {
      res.status(400).json({ error: "trace_pack.source must be one of: " + validSources.join(", ") })
      return
    }
    if (
      !rawTracePack.timestamp ||
      typeof rawTracePack.timestamp !== "string" ||
      isNaN(Date.parse(rawTracePack.timestamp))
    ) {
    ...
    if (rawTracePack.call_edges !== undefined && rawTracePack.call_edges !== null) {
      ...
      for (let i = 0; i < rawTracePack.call_edges.length; i++) {
        ...
        if (
          !edge ||
          typeof edge.caller_key !== "string" ||
          typeof edge.callee_key !== "string" ||
          typeof edge.call_count !== "number" ||
          !Number.isInteger(edge.call_count) ||
          edge.call_count < 1
        ) {
          res
            .status(400)
            .json({
              error: `trace_pack.call_edges[${i}] must have caller_key:string, callee_key:string, call_count:integer>=1`,
            })
          return
        }
      }
...
import { getRetentionStats, listStaleTransactions } from "../services/retention-service"
