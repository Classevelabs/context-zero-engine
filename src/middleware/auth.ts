...
function getClientFingerprint(req: Request): string {
  const keyHash = (() => {
    ...
    return key ? crypto.createHash("sha256").update(key).digest("hex").substring(0, 16) : "nokey"
  })()
...
function recordFailureInMap(map: Map<string, FailureRecord>, key: string, label: string): void {
...
export function authMiddleware(req: Request, res: Response, next: NextFunction): void {
  ...
  if (isThrottled(clientIp, fingerprint)) {
    ...
    const fpRetry = fpRecord && fpRecord.lockedUntil > 0 ? Math.max(0, fpRecord.lockedUntil - Date.now()) : 0
