...
export const optionalBoundedInt =
...
export const requireBoundedInt =
  (min: number, max: number) =>
  (value: unknown): string | null => {
    if (value === undefined || value === null) return "required"
    if (!isBoundedNumber(value, min, max)) return `must be a number between ${min} and ${max}`
...
export const requireStringArray =
  (maxLen: number = MAX_CHANGED_PATHS) =>
  (value: unknown): string | null => {
    if (value === undefined || value === null) return "required"
...
export const requirePatchArray = (value: unknown): string | null => {
  ...
  for (let i = 0; i < value.length; i++) {
    ...
    // Canonicalize Windows separators BEFORE traversal checks — engines on
    // Windows hand out backslash paths, and `..\..\` normalizes to `../../`
...
export const requireSafePathArray =
  (maxLen: number = MAX_CHANGED_PATHS) =>
  (value: unknown): string | null => {
    ...
    for (let i = 0; i < (value as string[]).length; i++) {
      ...
      // Canonicalize Windows separators, then traversal-check the result
