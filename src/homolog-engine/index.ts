...
/** Bounded scoring parallelism — each scoreCandidate can issue DB queries. */
...
export class HomologInferenceEngine {
  ...
  public async findHomologs(
    ...
    // be bounded or a large candidate set floods the pool queue and the driver's
    // overload rejection kills the whole call.
  ...
  public async persistHomologs(
    ...
    await db.transaction(async (client: PoolClient) => {
      for (const hom of homologs) {
        ...
        const bundleInsert = await db.queryWithClient(
          client,
          `
        ...
        if (bundleInsert.rows.length > 0) {
          ...
          const existing = await db.queryWithClient(
            client,
            `
        ...
        await db.queryWithClient(
          ...
          [
            relationId,
            sourceSymbolVersionId,
            hom.symbol_version_id,
            hom.relation_type,
            hom.confidence,
            bundleId,
            snapshotId,
   ...
   * Generate candidates from 5 buckets.
   */
  private async generateCandidates(target: CandidateRow, snapshotId: string): Promise<CandidateRow[]> {
    ...
    try {
      const semanticCandidates = await semanticEngine.findSemanticCandidates(target.symbol_version_id, snapshotId, 30)
      const newSvIds = semanticCandidates.map((sc) => sc.svId).filter((svId) => !candidateMap.has(svId))
  ...
  private async scoreCandidate(
    ...
    if (target.body_hash === candidate.body_hash) {
      ...
      target.normalized_ast_hash &&
      candidate.normalized_ast_hash &&
      target.normalized_ast_hash === candidate.normalized_ast_hash
    ) {
      logicSim = 0.9 // Rename-invariant structural match
      ...
      try {
        logicSim = await semanticEngine.computeBodySimilarity(target.symbol_version_id, candidate.symbol_version_id)
      } catch (error) {
    ...
    const behavioralOverlap = await this.computeBehavioralOverlap(target.symbol_version_id, candidate.symbol_version_id)
   ...
   */
  private async detectContradictions(target: CandidateRow, candidate: CandidateRow): Promise<string[]> {
    const flags: string[] = []
    ...
    if (targetBp && candidateBp) {
      ...
      if (
        sortedTargetAuth.length !== sortedCandidateAuth.length ||
      ...
      const targetIo = [...(targetBp.db_reads || []), ...(targetBp.db_writes || []), ...(targetBp.network_calls || [])]
        .sort()
        .join(",")
      const candidateIo = [
        ...(candidateBp.db_reads || []),
        ...(candidateBp.db_writes || []),
        ...(candidateBp.network_calls || []),
      ]
        .sort()
        .join(",")
  ...
  private async classifyRelationType(
    ...
    if (evidence.behavioral_overlap >= 0.6) {
      const [targetBp, candidateBp] = await Promise.all([
        this.loadBehavioralProfile(target.symbol_version_id),
        this.loadBehavioralProfile(candidate.symbol_version_id),
      ])
    ...
    if (evidence.contract_overlap >= 0.5) {
      const [targetBp, candidateBp] = await Promise.all([
        this.loadBehavioralProfile(target.symbol_version_id),
        this.loadBehavioralProfile(candidate.symbol_version_id),
      ])
  ...
  private async computeBehavioralOverlap(svIdA: string, svIdB: string): Promise<number> {
    const [a, b] = await Promise.all([this.loadBehavioralProfile(svIdA), this.loadBehavioralProfile(svIdB)])
    if (!a || !b) return 0.0
  ...
  private async computeContractOverlap(svIdA: string, svIdB: string): Promise<number> {
    const [a, b] = await Promise.all([this.loadContractProfile(svIdA), this.loadContractProfile(svIdB)])
    if (!a || !b) return 0.0
  ...
  private async loadBehavioralProfile(svId: string): Promise<BehavioralProfile | null> {
    ...
    const result = await db.query(`SELECT * FROM behavioral_profiles WHERE symbol_version_id = $1`, [svId])
    const profile = result.rows[0] ? validateBehavioralProfile(result.rows[0] as Record<string, unknown>) : null
  ...
  private async loadContractProfile(svId: string): Promise<ContractProfile | null> {
    ...
    const result = await db.query(`SELECT * FROM contract_profiles WHERE symbol_version_id = $1`, [svId])
    const profile = result.rows[0] ? validateContractProfile(result.rows[0] as Record<string, unknown>) : null
    if (profile) profileCache.set(cacheKey, profile)
    return profile
  }
  ...
  private buildRationale(
    ...
    if (semantic >= 0.7) parts.push("strong name similarity")
    if (sig >= 0.7) parts.push("matching signatures")
    if (behavioral >= 0.7) parts.push("overlapping behavior")
    if (contract >= 0.7) parts.push("matching contracts")
