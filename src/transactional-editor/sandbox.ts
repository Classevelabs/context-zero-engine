...
export function buildSanitizedEnv(cwd: string, extra?: Record<string, string>): Record<string, string> {
  ...
  const ALLOWED_VARS = ["PATH", "LANG", "LC_ALL", "LC_CTYPE", "TERM", "NODE_ENV"]
...
/**
 * Resolve a Node CLI to a direct `node <entry.js>` invocation instead of `npx`.
 *
 * `spawn("npx", ...)` is broken on Windows: npx is `npx.cmd`, and Node refuses
 * to spawn .cmd/.bat without shell:true (CVE-2024-27980), which we won't use
 * because sandboxed args include repo file paths. Resolving the package's JS
 * entry and running it with our own node binary works identically on every
 * platform — and skips npx's install-on-miss behavior (a sandbox should never
 * download packages anyway).
 *
 * Resolution order: the target project's node_modules, then our own.
 * Returns null when the package isn't installed anywhere visible.
 */
export function resolveNodeCli(entryModule: string, projectPath?: string): string | null {
  const searchPaths = [
    ...(projectPath ? [projectPath] : []),
    __dirname, // walk up through the engine's own node_modules
  ]
  for (const base of searchPaths) {
    try {
      return require.resolve(entryModule, { paths: [base] })
    } catch {
      // keep looking
    }
  }
 ...
 */
export async function sandboxTypeCheck(projectPath: string, tsconfigPath?: string): Promise<SandboxResult> {
  ...
  const tscJs = resolveNodeCli("typescript/lib/tsc.js", projectPath)
  if (!tscJs) {
    return {
      exitCode: 127,
      stdout: "",
      stderr: "TypeScript compiler not found (looked in project and engine node_modules)",
      timedOut: false,
      killed: false,
      durationMs: 0,
    }
  }
  return sandboxExec(process.execPath, [tscJs, "--noEmit", "--project", effectiveTsconfig], {
    cwd: projectPath,
...
export async function sandboxRunTests(
  ...
  switch (framework) {
    case "jest": {
      const jestJs = resolveNodeCli("jest/bin/jest.js", projectPath)
      if (!jestJs) {
        return {
          exitCode: 127,
          stdout: "",
          stderr: "jest not found (looked in project and engine node_modules)",
          timedOut: false,
          killed: false,
          durationMs: 0,
        }
      ...
      args = [jestJs, "--passWithNoTests", "--no-coverage", "--forceExit", ...testPaths]
      break
    }
    case "mocha": {
      const mochaJs = resolveNodeCli("mocha/bin/mocha.js", projectPath)
      if (!mochaJs) {
        return {
          exitCode: 127,
          stdout: "",
          stderr: "mocha not found (looked in project and engine node_modules)",
          timedOut: false,
          killed: false,
          durationMs: 0,
        }
      ...
      command = process.execPath
      args = [mochaJs, "--timeout", "30000", ...testPaths]
      break
    }
    case "pytest":
 ...
 */
export async function sandboxPythonCheck(projectPath: string, filePaths: string[]): Promise<SandboxResult> {
