 ...
 * it silently excluded an entire monorepo's source from native discovery. Skip it ONLY when it
 * carries an actual NuGet signature in its immediate children (mirrors ingestor/index.ts).
 */
...
/** Canonical repo-relative form: forward slashes on every platform. */
...
export async function ensureAllowedRepoPath(repoPath: string, options?: AllowedRepoPathOptions): Promise<string> {
  ...
  try {
    ...
    throw new Error(
      `repo_path does not exist or is not accessible: ${error instanceof Error ? error.message : String(error)}`,
    )
  }
  ...
  const walk = async (currentDir: string): Promise<void> => {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (entry.name === "packages" && (await isNugetPackagesDir(entryPath))) continue
...
async function computeWorkspaceFingerprint(
  repoPath: string,
): Promise<{ commit_sha: string; files_considered: number; truncated: boolean }> {
...
function summarizeRiskHints(behaviorHints: BehaviorHint[], stableKey: string): string[] {
...
function scoreSymbol(query: string, symbol: ExtractedSymbol, relativePath: string): number {
  ...
  const tokenHits = queryTokens.filter(
    (token) => normalizedName.includes(token) || normalizedStableKey.includes(token) || normalizedPath.includes(token),
  ).length
  if (tokenHits > 0) {
...
export async function buildNativeCodebaseOverview(
  ...
  for (const file of discovery.files) {
    const directory = path.posix.dirname(file.relativePath) === "." ? "." : path.posix.dirname(file.relativePath)
