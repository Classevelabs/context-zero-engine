 ...
 * canonical source root of every JS/TS monorepo (bun/npm/yarn/pnpm workspaces). Blanket-skipping
 * it silently excluded an entire monorepo's source from the index (observed live: 178 files indexed
 * out of two thousand+, zero symbols from packages/). Skip it ONLY when it carries an actual NuGet
 * signature in its immediate children: .nupkg payloads or the legacy repositories.config.
 */
async function isNugetPackagesDir(dirPath: string): Promise<boolean> {
  try {
    const entries = await fsp.readdir(dirPath, { withFileTypes: true })
    return entries.some(
      (e) =>
        (e.isFile() && (e.name === "repositories.config" || e.name.endsWith(".nupkg"))) ||
        // extracted NuGet layout: immediate children named Package.Name.1.2.3
        (e.isDirectory() && /\.\d+\.\d+(\.\d+)?([-.].+)?$/.test(e.name)),
    )
  } catch {
    return false // unreadable — treat as source and let the walk decide per-entry
  }
}
...
export class Ingestor {
    ...
    try {
      ...
      if (parentSnapshotId) {
        try {
          ...
          const parentStatusResult = await db.query(`SELECT index_status FROM snapshots WHERE snapshot_id = $1`, [
            parentSnapshotId,
          ])
          ...
          if (parentStatus !== "complete") {
            ...
            const parentFilesResult = await db.query(`SELECT path, content_hash FROM files WHERE snapshot_id = $1`, [
              parentSnapshotId,
            ])
      ...
      if (tsPaths.length > 0) {
        ...
        try {
          ...
          filesProcessed += tsPaths.length - tsFailed
          filesFailed += tsFailed
          if (tsFailed > 0) {
            failureSummary.push(
          ...
          // reaching here means persistence (or something systemic) threw —
          // record WHY, not just a count. Extraction work is lost for all
          // TS files this run.
          const msg = err instanceof Error ? err.message : String(err)
      ...
      const treeSitterPaths: {
        filePath: string
        lang: "cpp" | "go" | "rust" | "java" | "csharp" | "ruby" | "kotlin" | "swift" | "php" | "bash"
      }[] = [
      ...
      if (treeSitterPaths.length > 0) {
        ...
        const CONCURRENCY = Math.min(INGEST_WORKER_CONCURRENCY, treeSitterPaths.length)
        ...
        for (const { result, lang, filePath } of extractionResults) {
          if (result) {
            try {
      ...
      if (!extractionFailed) {
        ...
        try {
          ...
          log.warn("V2: Dispatch resolution failed (non-fatal)", {
            error: err instanceof Error ? err.message : String(err),
          })
        }
        ...
        try {
          ...
          log.warn("V2: Deep contract synthesis failed (non-fatal)", {
            error: err instanceof Error ? err.message : String(err),
          })
        }
        ...
        try {
          ...
          log.warn("V2: Concept family engine failed (non-fatal)", {
            error: err instanceof Error ? err.message : String(err),
          })
        }
        ...
        try {
          ...
          log.warn("V2: Temporal engine failed (non-fatal)", {
            error: err instanceof Error ? err.message : String(err),
          })
        }
      ...
      if (languageSet.size > 0) {
        await db.query(`UPDATE repositories SET language_set = $1, updated_at = NOW() WHERE repo_id = $2`, [
      ...
      const result: IngestionResult = {
        ...
        duration_ms: Date.now() - startTime,
        ...(failureSummary.length > 0 ? { failure_summary: failureSummary.slice(0, 10) } : {}),
        // V2 additions
  ...
  private async persistExtractionResult(
    ...
    const getFileLines = async (filePath: string): Promise<string[] | null> => {
      ...
      try {
        ...
        log.warn("Failed to read file for body_source extraction", {
          filePath: abs,
          error: err instanceof Error ? err.message : String(err),
        })
    ...
    const normalizeKey = (key: string): string => {
      ...
      // Always emit the portable (forward-slash) form — files.path is stored
      // portable, and a separator mismatch silently orphans every symbol.
    ...
    const fileResultAll = await db.query(`SELECT file_id, path FROM files WHERE snapshot_id = $1`, [snapshotId])
    ...
    const canonicalNameForDb = (name: string): string => {
      const normalized = name.replace(/\s+/g, " ").trim()
      if (normalized.length <= MAX_CANONICAL_NAME_LENGTH) return normalized
      return `${normalized.slice(0, MAX_CANONICAL_NAME_LENGTH - 3)}...`
    }
    const distinctSymInputs = new Map<
      string,
      {
        stable_key: string
    ...
    for (const sym of extraction.symbols) {
      ...
      const portableKeyPath = toPortableRelativePath(
        path.isAbsolute(stableKeyPath) ? path.relative(repoPath, stableKeyPath) : stableKeyPath,
      )
      if (portableKeyPath !== stableKeyPath) {
        stableKeyPath = portableKeyPath
    ...
    if (allEntries.length > 0) {
      ...
      const RESOLVE_CHUNK_SIZE = 30_000
      for (let offset = 0; offset < allEntries.length; offset += RESOLVE_CHUNK_SIZE) {
        const chunk = allEntries.slice(offset, offset + RESOLVE_CHUNK_SIZE)
        const symbolIds = chunk.map((e) => e.symbolId)
  ...
  private async cleanupSnapshotData(snapshotId: string): Promise<void> {
    ...
    await db.transaction(async (client: PoolClient) => {
      ...
      const svResult = await db.queryWithClient(
        client,
      ...
      if (svIds.length > 0) {
        ...
        await db.queryWithClient(client, "DELETE FROM symbol_versions WHERE snapshot_id = $1", [snapshotId])
      }
  ...
  private async extractWithUniversalAdapter(
    ...
    try {
      ...
      const relativePath = toPortableRelativePath(path.relative(repoPath, filePath))
    ...
    const walk = async (dir: string): Promise<void> => {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          // "packages" is only a cache dir in the NuGet world; in a JS/TS monorepo it IS
          // the source tree. Decide from the directory's contents, not its name.
          if (entry.name === "packages" && (await isNugetPackagesDir(entryPath))) {
            log.info("Skipping NuGet packages directory", { path: entryPath })
            continue
          }
  ...
  public async ingestIncremental(
    ...
    if (!incrementalLock) {
      ...
      return {
        ...
        deep_contracts_mined: 0,
        concept_families_built: 0,
        temporal_co_changes_found: 0,
      }
    }
    ...
    try {
      ...
      // Wrapped in a single DB transaction for atomicity. Paths with an
      // existing file row are remembered so step 2 can tell an edit (refresh)
      ...
      await db.transaction(async (client: PoolClient) => {
        for (const changedPath of changedPaths) {
          ...
          if (fileId) {
            ...
            await db.queryWithClient(client, `DELETE FROM symbol_versions WHERE file_id = $1 AND snapshot_id = $2`, [
              fileId,
              snapshotId,
            ])
          }
      ...
      for (const changedPath of changedPaths) {
        ...
        // Upsert the file row. A path NEW since the snapshot used to hit a
        // bare UPDATE here that matched zero rows — extraction then ran into
        // the void because persistExtractionResult drops any symbol whose
        // file row is missing (fileIdByPath miss), while the success counts
        // still reported the collateral work. Same addFile as the full
        // ingest: an edit refreshes the hash, a first-seen path gets a real
        // file row so its symbols land like any other change.
        ...
        try {
          ...
          if (!knownPaths.has(changedPath)) {
            newFilesIndexed++
            log.info("Incremental: new-since-snapshot file registered", { path: changedPath, lang })
          }
        } catch (err) {
          // One file's upsert failing (transient DB hiccup) must not abort the
          // whole incremental batch — fall back to the old best-effort refresh:
          // a KNOWN file still gets its hash updated; a NEW file stays invisible
          // this round and rides the next change or a full ingest.
          log.warn("Incremental: file upsert failed; falling back to bare refresh", {
            path: changedPath,
            error: err instanceof Error ? err.message : String(err),
          })
          await db.query(
  ...
  private async findTsconfig(repoPath: string): Promise<string | null> {
    const tsconfig = repoPath.includes("\\")
      ? path.join(repoPath, "tsconfig.json")
      : path.posix.join(repoPath, "tsconfig.json")
    try {
