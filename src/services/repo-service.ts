 ...
 */
export async function listRepos(limit: number = 20, offset: number = 0): Promise<ListReposResult> {
  const result = await db.query(`SELECT * FROM repositories ORDER BY updated_at DESC LIMIT $1 OFFSET $2`, [
    limit,
    offset,
  ])
