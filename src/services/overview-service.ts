...
export async function getCodebaseOverview(snapshotId: string): Promise<CodebaseOverview> {
  ...
  const [langResult, pathResult] = await Promise.all([
    db.query(`SELECT language, COUNT(*) as count FROM files WHERE snapshot_id = $1 GROUP BY language`, [snapshotId]),
    db.query(`SELECT path FROM files WHERE snapshot_id = $1`, [snapshotId]),
  ])
  ...
  for (const row of (pathResult?.rows ?? []) as { path: string }[]) {
    ...
    // Accept both separators — pre-fix Windows snapshots stored backslash paths,
    // which made every file bucket into "." here.
    const portable = row.path.replace(/\\/g, "/")
    const dir = portable.split("/").slice(0, -1).join("/") || "."
  ...
  const publicSymbols: string[] = (
    publicResult.rows as { kind: string; canonical_name: string; file_path: string }[]
  ...
  const purityDist = Object.fromEntries(
    ((behaviorResult?.rows ?? []) as { purity_class: string; cnt: string }[]).map((r) => {
  ...
  return {
    summary: {
      ...
      directories: Object.entries(dirCounts)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 20),
    },
    symbols: {
    ...
    test_coverage: {
      ...
      coverage_percent: totalSymbols > 0 ? ((testedCount / totalSymbols) * 100).toFixed(1) + "%" : "0%",
    },
    uncertainty: {
