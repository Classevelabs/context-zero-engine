...
import { firstRow, stringArrayField, parseCountField } from "../db-driver/result"
 ...
 * severity.
 */
export async function computeSemanticDiff(options: SemanticDiffOptions): Promise<SemanticDiffResult> {
  ...
  changes.push({
    ...
    before: beforePurity,
    after: afterPurity,
    severity: !purityChanged ? "none" : purityEscalated ? "breaking" : "minor",
  ...
  changes.push({
    ...
    severity: !sideEffectsChanged ? "none" : newSideEffects.length > 0 ? "major" : "minor",
  ...
  changes.push({
    ...
    severity: !exceptionsChanged ? "none" : newExceptions.length > 0 ? "major" : "minor",
  ...
  changes.push({
    ...
    severity: !authChanged ? "none" : removedAuth.length > 0 ? "breaking" : "minor",
    detail: !authChanged
  ...
  changes.push({
    ...
    detail: serializationChanged ? `Serialization contract changed` : "Serialization unchanged",
  })
  ...
  const persistenceChanged = !arraysEqual(beforeDbReads, afterDbReads) || !arraysEqual(beforeDbWrites, afterDbWrites)
  ...
  changes.push({
    ...
    severity: !persistenceChanged ? "none" : newDbWrites.length > 0 ? "major" : "minor",
  ...
  changes.push({
    ...
    severity: !validationChanged ? "none" : removedValidation.length > 0 ? "major" : "minor",
  ...
  changes.push({
    ...
    severity: !resourceChanged ? "none" : newResources.length > 0 ? "major" : "minor",
  ...
  if (changedDims.length === 0) {
    ...
    if (hasBreaking) {
      const breakingDims = changes.filter((c) => c.severity === "breaking").map((c) => c.dimension)
      parts.push(`BREAKING in: ${breakingDims.join(", ")}`)
    }
    const majorDims = changes.filter((c) => c.severity === "major").map((c) => c.dimension)
 ...
 */
export async function computeContractDiff(options: ContractDiffOptions): Promise<ContractDiffResult> {
  ...
  if (options.txn_id) {
    ...
    if (targetVersions.length === 0) {
      throw UserFacingError.badRequest("Transaction has no target symbol versions to diff")
    }
    ...
    if (!afterSvRow) {
      throw UserFacingError.notFound(`Target symbol version ${afterSvId}`)
    }
    ...
    if (!beforeRow) {
      throw UserFacingError.notFound(`Base snapshot version for symbol ${symbolId} in snapshot ${baseSnapshotId}`)
    }
    ...
    throw UserFacingError.badRequest("Provide either before/after symbol version IDs or a transaction ID")
  }
  ...
  if (!scopeSymbolId) {
    const svResult = await db.query(`SELECT symbol_id FROM symbol_versions WHERE symbol_version_id = $1`, [afterSvId])
  ...
  if (scopeSymbolId) {
    const invResult = await db.query(`SELECT COUNT(*)::int AS cnt FROM invariants WHERE scope_symbol_id = $1`, [
      scopeSymbolId,
    ])
  ...
  if (changedFields.length === 0) {
    ...
    if (hasBreaking) {
      const breakingFields = changes.filter((c) => c.severity === "breaking").map((c) => c.field)
