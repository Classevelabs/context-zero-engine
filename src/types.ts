...
export type RelationProvenance =
  ...
  | "framework_declared"
  | "developer_asserted"
export type InvariantSourceType =
  | "explicit_test"
  | "derived"
  | "manual"
  | "assertion"
...
export interface AdapterExtractionResult {
  ...
  uncertainty_flags: string[]
  /** Files that could not be extracted (per-file isolation) — absent/empty when all succeeded. */
  failed_files?: string[]
}
...
export const HOMOLOG_WEIGHTS = {
  semantic_intent_similarity: 0.2,
  normalized_logic_similarity: 0.2,
...
export const DEFAULT_HOMOLOG_CONFIDENCE_THRESHOLD = 0.6
