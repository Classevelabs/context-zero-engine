...
function safeTool(handler: ToolHandler): (args: Record<string, unknown>) => Promise<McpCallToolResult> {
  return async (args: Record<string, unknown>) => {
    try {
      ...
      // UserFacingError is BUILT to be shown to callers (400/403/404/422) —
      // swallowing it into "Internal server error" turns every bad-input case
      // into an undiagnosable fake 500. Expose it; sanitize everything else
      // unless it matches a known safe prefix.
      const isSafe =
        err instanceof UserFacingError || SAFE_ERROR_PREFIXES.some((prefix) => rawMessage.startsWith(prefix))
      ...
      if (typeof token !== "string" || !secretsMatch(token, requiredSecret)) {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: JSON.stringify({ error: "Authentication failed: invalid or missing _auth_token" }),
            },
          ],
          isError: true,
        }
      ...
      if (!rateLimiters[name].check(name)) {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: JSON.stringify({ error: `Rate limit exceeded for ${name}. Please wait and try again.` }),
            },
          ],
          isError: true,
        }
...
registerTool(
  ...
  {
    description:
      "Fuzzy symbol search — find symbols by name in a repository. Returns ranked matches with similarity scores.",
    inputSchema: {
...
registerTool(
  "scg_get_symbol_details",
  {
    ...
    inputSchema: {
      ...
      view_mode: z
        .enum(["code", "summary", "signature"])
        .optional()
        .describe("Detail level: code (full), summary (with profiles), or signature (minimal). Default: summary"),
    },
  },
  ...
  "scg_get_symbol_relations",
  {
    ...
    inputSchema: {
      ...
      direction: z
        ...
        .optional()
        .describe("Relation direction filter. Default: both"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Get the behavioral profile of a symbol — purity class, resource touches, DB ops, network calls, side effects.",
    inputSchema: {
      symbol_version_id: z.string().uuid().describe("Symbol version UUID"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Get the contract profile of a symbol — input/output contracts, error contracts, schema refs, security contract.",
    inputSchema: {
      symbol_version_id: z.string().uuid().describe("Symbol version UUID"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Get the uncertainty report for a snapshot — areas where analysis confidence is low or evidence is insufficient.",
    inputSchema: {
...
registerTool(
  ...
  {
    description:
      "Find homologous symbols — code clones, near-duplicates, validators with parallel logic, co-changed peers.",
    inputSchema: {
      ...
      confidence_threshold: z
        ...
        .max(1)
        .optional()
        .describe("Minimum confidence score (0-1). Default: 0.70"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Compute blast radius — impact analysis showing structural, behavioral, contract, and homolog impacts of changing symbols.",
    inputSchema: {
      symbol_version_ids: z
        .array(z.string().uuid())
        .min(1)
        .describe("Array of symbol version UUIDs to analyze impact for"),
      snapshot_id: z.string().uuid().describe("Snapshot UUID"),
...
registerTool(
  ...
  {
    description:
      "Compile a token-budgeted context capsule for a symbol — includes code, dependencies, callers, tests, contracts, and homologs, prioritized to fit within a token budget.",
    inputSchema: {
      ...
      mode: z
        ...
        .optional()
        .describe("Capsule compilation mode. Default: standard"),
      token_budget: z
        .number()
        .int()
        .min(100)
  ...
  "scg_apply_patch",
  {
    ...
    inputSchema: {
      ...
      patches: z
        ...
        .min(1)
        .describe("Array of file patches to apply"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Run 6-level validation on a change transaction — syntax, semantic, contract, invariant, behavioral, and propagation checks.",
    inputSchema: {
      txn_id: z.string().uuid().describe("Transaction UUID"),
...
registerTool(
  ...
  {
    description:
      "Compute homolog co-change proposals — suggests changes to homologous symbols that may need parallel updates.",
    inputSchema: {
      txn_id: z.string().uuid().describe("Transaction UUID"),
      snapshot_id: z.string().uuid().describe("Snapshot UUID"),
    },
...
registerTool(
  ...
  {
    description:
      "Register a repository for indexed workflows. In MCP mode, if SCG_ALLOWED_BASE_PATHS is unset, access falls back to the current working directory only.",
    inputSchema: {
      repo_name: z.string().describe("Human-readable repository name"),
      repo_path: z
        .string()
        .refine((v) => path.isAbsolute(v), { message: "repo_path must be an absolute path" })
...
registerTool(
  ...
  {
    description:
      "Ingest (index) a repository by registered repo_id or direct repo_path. If commit_sha is omitted, MCP derives it from git HEAD or a workspace fingerprint for non-git repos.",
    inputSchema: {
      ...
      branch: z
        .string()
        .optional()
        .describe("Branch name. Default: repo default branch, git branch, or main/workspace"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Discover and persist homolog relations for a symbol — runs homolog detection and saves results to the database.",
    inputSchema: {
      ...
      confidence_threshold: z
        ...
        .max(1)
        .optional()
        .describe("Minimum confidence threshold (0-1). Default: 0.70"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Read source code from DB — by symbol_version_id (single), symbol_version_ids (batch, up to 20), or file_path (disk). Batch mode returns all symbols in one call (~500 tokens each vs ~5000 for full file). Prefer batch mode for multi-symbol lookups.",
    inputSchema: {
      repo_id: z.string().uuid().describe("Repository UUID"),
      ...
      symbol_version_ids: z
        ...
        .max(20)
        .optional()
        .describe("Batch: array of symbol version UUIDs (max 20)"),
      ...
      context_lines: z
        ...
        .max(50)
        .optional()
        .describe("Extra context lines around symbol (default 0)"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Search/grep across all indexed files in a repository. Returns matching lines with surrounding context. Supports regex patterns. Essential for finding implementations, usages, and patterns during audits.",
    inputSchema: {
      repo_id: z.string().uuid().describe("Repository UUID"),
      pattern: z
        .string()
        .describe('Search pattern (regex supported). E.g., "async function train", "class.*Block", "def forward"'),
      file_pattern: z
        .string()
        .optional()
        .describe('Filter to files matching this substring. E.g., ".py", "src/model", "test"'),
      max_results: z
        .number()
        .int()
        .min(1)
        .max(100)
        .optional()
        .describe("Maximum matches to return (default 30, max 100)"),
      context_lines: z
        .number()
        .int()
        .min(0)
        .max(5)
        .optional()
        .describe("Lines of context around each match (default 2, max 5)"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "High-level architecture summary with risk assessment. Shows: file structure, symbol distribution, behavioral purity profile, test coverage gaps, high-risk symbols (side-effecting, network, DB), and uncertainty analysis. Use this FIRST when auditing an unfamiliar codebase.",
    inputSchema: {
      repo_id: z.string().uuid().describe("Repository UUID"),
      snapshot_id: z.string().uuid().describe("Snapshot UUID"),
    },
...
registerTool(
  ...
  {
    description:
      "Zero-setup codebase overview for a local repo path. Works before registration or ingestion. Supports 13 languages: TypeScript, JavaScript, Python, C/C++, Go, Rust, Java, C#, Ruby, Kotlin, Swift, PHP, Bash.",
    inputSchema: {
...
registerTool(
  ...
  {
    description:
      "Zero-setup symbol search over a local repo path. Parses supported languages directly from disk and does not require git, registration, or prior ingestion.",
    inputSchema: {
      repo_path: z.string().describe("Absolute or relative repository path"),
      ...
      max_results: z
        ...
        .max(100)
        .optional()
        .describe("Maximum symbol matches to return (default 20)"),
...
registerTool(
  ...
  {
    description:
      "Zero-setup grep across a local repo path. Searches readable text files directly from disk with regex safety guards and surrounding context.",
    inputSchema: {
      repo_path: z.string().describe("Absolute or relative repository path"),
      pattern: z
        .string()
        .max(500)
        .describe("Search pattern (regex supported; unsafe patterns fall back to literal search)"),
...
registerTool(
  ...
  {
    description:
      'Search inside function bodies using semantic similarity (TF-IDF). Unlike resolve_symbol (name-only), this finds code by what it DOES. Query: "accumulate V×V matrices" or "retry with exponential backoff". Returns ranked symbols with source.',
    inputSchema: {
...
registerTool(
  ...
  {
    description:
      "Get everything needed for a change task in ONE call. Provide target symbols + task description → returns: target source, blast radius impacts with source, homologs, tests — all token-budgeted. Replaces 8+ separate tool calls.",
    inputSchema: {
      task_description: z.string().describe("What change are you making and why"),
      target_symbol_version_ids: z
        .array(z.string().uuid())
        .min(1)
        .max(20)
        .describe("Symbol version UUIDs being changed"),
      snapshot_id: z.string().uuid().describe("Snapshot UUID"),
      token_budget: z
        .number()
        .int()
        ...
        .max(100000)
        .optional()
        .describe("Max tokens for response (default 20000)"),
...
registerTool(
  ...
  {
    description:
      "Get resolved dispatch edges for a symbol — shows how object method chains (self.service.validate()) are resolved to actual target symbols.",
    inputSchema: {
      symbol_version_id: z.string().uuid().describe("Symbol version UUID"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Get the class hierarchy and Method Resolution Order (MRO) for a class symbol — shows inheritance chain, interfaces, and override relationships.",
    inputSchema: {
      snapshot_id: z.string().uuid().describe("Snapshot UUID"),
...
registerTool(
  ...
  {
    description:
      "Get the lineage history of a symbol across snapshots — tracks identity through renames, moves, and refactors.",
    inputSchema: {
...
registerTool(
  ...
  {
    description:
      "Get the typed effect signature for a symbol — precise reads/writes/emits/calls/mutates/requires/throws effects with resources and confidence.",
    inputSchema: {
      symbol_version_id: z.string().uuid().describe("Symbol version UUID"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Compare effect signatures between two symbol versions — shows added, removed, and changed effects for change validation.",
    inputSchema: {
...
registerTool(
  ...
  {
    description:
      "Get the concept family a symbol belongs to — shows family type, exemplar, members, outliers, and contradictions.",
    inputSchema: {
      symbol_version_id: z.string().uuid().describe("Symbol version UUID"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "List all concept families in a snapshot — validator families, serializer families, auth policy families, etc.",
    inputSchema: {
      snapshot_id: z.string().uuid().describe("Snapshot UUID"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Get temporal risk score for a symbol — based on change frequency, bug-fix history, regression count, churn, and ownership patterns.",
    inputSchema: {
      symbol_id: z.string().uuid().describe("Symbol UUID"),
      snapshot_id: z.string().uuid().describe("Snapshot UUID"),
    },
...
registerTool(
  ...
  {
    description:
      "Ingest runtime trace data — test execution traces, dev run traces, or CI traces. Observed call edges are merged with the static graph to reduce uncertainty and improve dispatch resolution.",
    inputSchema: {
      repo_id: z.string().uuid().describe("Repository UUID"),
      snapshot_id: z.string().uuid().describe("Snapshot UUID"),
      trace_pack: z
        .object({
          ...
          call_edges: z
            ...
            .max(50000)
            .describe("Observed call edges (max 50000)"),
          dynamic_routes: z
            .array(
              z.object({
                route: z.string(),
            ...
            .max(10000)
            .default([])
            .describe("Dynamic route registrations (max 10000)"),
          observed_types: z
            .array(
              z.object({
                expression: z.string(),
            ...
            .max(10000)
            .default([])
            .describe("Observed runtime types (max 10000)"),
          framework_events: z
            .array(
              z.object({
                event_type: z.string(),
            ...
            .max(10000)
            .default([])
            .describe("Framework-specific events (max 10000)"),
        })
...
registerTool(
  ...
  {
    description:
      "Get runtime evidence for a symbol — observed call edges, caller observations, and confidence boost from runtime data.",
    inputSchema: {
      symbol_version_id: z.string().uuid().describe("Symbol version UUID"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Health check — returns DB connection status (with latency), pool stats, migration count, server uptime, and version. Use to verify the server is operational before starting a task.",
    inputSchema: {},
  },
...
registerTool(
  ...
  {
    description:
      "Batch-embed all symbol versions in a snapshot — generates semantic vectors for similarity search. Run after ingestion to enable scg_semantic_search.",
    inputSchema: {
      snapshot_id: z.string().uuid().describe("Snapshot UUID"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Returns hit/miss/eviction stats for all 5 in-process caches (symbol, profile, capsule, homolog, query). Use to diagnose performance issues or verify cache warming.",
    inputSchema: {},
  },
...
registerTool(
  ...
  {
    description:
      "Get test artifacts related to a symbol — test names, frameworks, assertion summaries, and coverage hints.",
    inputSchema: {
      symbol_id: z.string().uuid().describe("Symbol UUID (not symbol_version_id)"),
      snapshot_id: z.string().uuid().describe("Snapshot UUID"),
    },
...
registerTool(
  ...
  {
    description:
      "Explain the relationship between two symbols — returns structural and inferred relations with evidence bundles and confidence scores.",
    inputSchema: {
...
registerTool(
  ...
  {
    description:
      "Get the graph neighborhood of a symbol — BFS traversal through structural relations with configurable depth and direction.",
    inputSchema: {
      ...
      relation_types: z
        .array(z.string())
        .optional()
        .describe('Filter by specific relation types (e.g., ["calls", "imports"])'),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Search for symbols by concept — combines name matching, semantic similarity, concept family lookup, and contract search. Returns ranked matches with relevance scores.",
    inputSchema: {
      concept: z
        .string()
        .min(1)
        .max(500)
        .describe('Concept search string (e.g., "email validation", "payment processing")'),
      repo_id: z.string().uuid().describe("Repository UUID"),
      snapshot_id: z.string().uuid().describe("Snapshot UUID"),
...
registerTool(
  ...
  {
    description:
      "Compare two symbol versions semantically — detects changes in side effects, return types, exceptions, auth behavior, serialization, and persistence across 9 dimensions.",
    inputSchema: {
...
registerTool(
  ...
  {
    description:
      "Compute contract delta between two symbol versions or for a transaction — detects breaking changes in input/output/error/security contracts.",
    inputSchema: {
...
registerTool(
  ...
  {
    description:
      "Plan a code change from natural language — resolves target symbols, computes initial blast radius, and recommends capsule mode. The entry point for the change workflow.",
    inputSchema: {
      repo_id: z.string().uuid().describe("Repository UUID"),
      snapshot_id: z.string().uuid().describe("Snapshot UUID"),
      ...
      max_candidates: z
        ...
        .max(20)
        .optional()
        .describe("Maximum target candidates (default 5, max 20)"),
      scope_constraints: z
        .object({
        ...
        .optional()
        .describe("Optional scope constraints to narrow candidate search"),
    },
  },
...
registerTool(
  ...
  {
    description:
      "Prepare a change transaction — locks target symbol versions and establishes preconditions before patching.",
    inputSchema: {
      repo_id: z.string().uuid().describe("Repository UUID"),
...
registerTool(
  ...
  {
    description:
      "Apply a propagation patch to a homolog target within an existing transaction — executes recommended changes from propagation proposals.",
    inputSchema: {
...
registerTool(
  ...
  {
    description:
      "Review an inferred homolog relation — approve, reject, or flag for further investigation. Updates the review state for future analysis.",
    inputSchema: {
...
registerTool(
  ...
  {
    description:
      "Run the retention policy immediately — expires old snapshots, enforces per-repo snapshot caps, cleans stale transactions, and removes orphaned data. Returns a summary of all cleanup operations performed.",
    inputSchema: {},
  },
...
registerTool(
  ...
  {
    description:
      "Get retention policy statistics — total/expired snapshots, stale transactions, oldest snapshot age, and last cleanup run timestamp.",
    inputSchema: {},
  },
...
registerTool(
  ...
  {
    description:
      "Clean up transactions stuck in intermediate states (planned, prepared, patched, etc.) beyond the configured timeout. Marks them as failed and returns the count of cleaned and remaining stale transactions.",
    inputSchema: {},
  },
...
registerTool(
  ...
  {
    description:
      "Get database health statistics — table sizes and row counts, least-used indexes, total database size, and connection pool state. Use to diagnose storage growth or index bloat.",
    inputSchema: {},
  },
...
registerTool(
  ...
  {
    description:
      "Get system-wide operational information — server uptime, memory usage, database entity counts (repos, snapshots, symbols, relations), connection health, and cache statistics across all 5 cache layers.",
    inputSchema: {},
  },
...
async function main(): Promise<void> {
  ...
  // Index size-management. The stdio bridge is the door agents actually index through, so
  // without this it would accumulate snapshots unboundedly (every re-ingest of every repo →
  // gigabytes of Postgres). The HTTP interface already schedules retention; the bridge did not.
  // A short-lived per-session bridge would rarely reach any interval, so the STARTUP prune is the
  ...
  if (retentionConfig.retentionEnabled) {
    ...
    log.info("Retention policy active", {
      intervalMinutes: retentionConfig.retentionIntervalMinutes,
      maxSnapshotsPerRepo: retentionConfig.maxSnapshotsPerRepo,
      snapshotMaxAgeDays: retentionConfig.snapshotMaxAgeDays,
    })
  }
