...
const IF_THROW =
  /if\s*\(\s*(![\w.[\]?]+|[\w.[\]?]+\s*(?:===?|!==?)\s*(?:null|undefined|false|0|''|""|``)|![\w.[\]?]+)\s*\)\s*\{?\s*throw\b/g
const IF_RETURN_GUARD =
  /if\s*\(\s*(![\w.[\]?]+|[\w.[\]?]+\s*(?:===?|!==?)\s*(?:null|undefined|false|0|''|""|``)|![\w.[\]?]+)\s*\)\s*\{?\s*return\b/g
const INVARIANT_CALL = /\binvariant\s*\(/g
const PRECONDITION_CALL = /\bprecondition\s*\(/g

// --- Type guard patterns ---
const TYPEOF_GUARD =
  /typeof\s+([\w.]+)\s*(?:===?|!==?)\s*['"](string|number|boolean|function|object|undefined|bigint|symbol)['"]/g
...
const RANGE_CHECK =
  /if\s*\(\s*([\w.]+)\s*(<|>|<=|>=)\s*(-?\d+(?:\.\d+)?)\s*(?:\|\||&&)\s*\1\s*(<|>|<=|>=)\s*(-?\d+(?:\.\d+)?)\s*\)/g
const MATH_CLAMP_MIN = /Math\s*\.\s*max\s*\(\s*(-?\d+(?:\.\d+)?)\s*,\s*([\w.]+)\s*\)/g
const MATH_CLAMP_MAX = /Math\s*\.\s*min\s*\(\s*(-?\d+(?:\.\d+)?)\s*,\s*([\w.]+)\s*\)/g
const MATH_CLAMP_FULL =
  /Math\s*\.\s*max\s*\(\s*(-?\d+(?:\.\d+)?)\s*,\s*Math\s*\.\s*min\s*\(\s*(-?\d+(?:\.\d+)?)\s*,\s*([\w.]+)\s*\)\s*\)/g
...
const MONGOOSE_SCHEMA_TYPE =
  /(\w+)\s*:\s*\{\s*type\s*:\s*(String|Number|Boolean|Date|Buffer|ObjectId|Array|Map|Schema\.Types\.\w+)/g
...
export class DeepContractSynthesizer {
   ...
   */
  public async synthesizeContracts(repoId: string, snapshotId: string): Promise<number> {
    ...
    const countResult = await db.query(`SELECT COUNT(*) as cnt FROM symbol_versions WHERE snapshot_id = $1`, [
      snapshotId,
    ])
   ...
   */
  public async mineCrossSymbolPatterns(repoId: string, snapshotId: string): Promise<number> {
      ...
      if (bucket.symbol_ids.length >= 10) {
        ...
        statements.push({
          ...
          params: [
            ...
            "derived",
            Math.min(0.98, boostedStrength + 0.05),
            "cross_symbol_pattern_mining",
            snapshotId,
          ],
        })
  ...
  private mineAssertions(body: string, symbolId: string, out: InvariantCandidate[]): void {
    // assert(expr)
  ...
  private mineGuardClauses(body: string, symbolId: string, out: InvariantCandidate[]): void {
  ...
  private mineTypeGuards(body: string, symbolId: string, out: InvariantCandidate[]): void {
    // typeof x === 'string'
  ...
  private mineRangeChecks(body: string, symbolId: string, out: InvariantCandidate[]): void {
  ...
  private mineRegexValidators(body: string, symbolId: string, out: InvariantCandidate[]): void {
    // /pattern/.test(x)
  ...
  private mineEnumRestrictions(body: string, symbolId: string, out: InvariantCandidate[]): void {
  ...
  private mineInputNormalization(body: string, symbolId: string, out: InvariantCandidate[]): void {
    // .trim()
  ...
  private mineOrmSchemaDefinitions(body: string, symbolId: string, out: InvariantCandidate[]): void {
  ...
  private mineZodYupJoiSchemas(body: string, symbolId: string, out: InvariantCandidate[]): void {
    // --- Zod ---
    const hasZod =
    ...
    // --- Yup ---
    const hasYup =
      body.includes("yup.string") ||
      body.includes("yup.number") ||
      body.includes("yup.boolean") ||
      body.includes("yup.object")
    ...
    // --- Joi ---
    const hasJoi =
      body.includes("Joi.string") ||
      body.includes("Joi.number") ||
      body.includes("Joi.boolean") ||
      body.includes("Joi.object")
  ...
  private mineClosureContracts(body: string, symbolId: string, out: InvariantCandidate[]): void {
    ...
    if (returnStatements.length > 0) {
      ...
      const hasNullReturn = returnValues.some((v) => v === "null" || v === "undefined" || v === "void 0")
      const hasPromiseReturn = returnValues.some((v) => /^(?:new\s+)?Promise|await\s/.test(v))
      const hasObjectReturn = returnValues.some((v) => v.startsWith("{"))
      const hasArrayReturn = returnValues.some((v) => v.startsWith("["))
      const hasBooleanReturn = returnValues.some((v) => v === "true" || v === "false")
      const hasNumericReturn = returnValues.some((v) => /^-?\d+(\.\d+)?$/.test(v))
  ...
  private mineParamTypes(signature: string, symbolId: string, out: InvariantCandidate[]): void {
  ...
  private mineReturnType(signature: string, symbolId: string, out: InvariantCandidate[]): void {
  ...
  private mineGenericConstraints(signature: string, symbolId: string, out: InvariantCandidate[]): void {
  ...
  private mineUnionTypes(signature: string, symbolId: string, out: InvariantCandidate[]): void {
  ...
  private mineOptionalParams(signature: string, symbolId: string, out: InvariantCandidate[]): void {
    const parenStart = signature.indexOf("(")
    const parenEnd = signature.lastIndexOf(")")
    if (parenStart === -1 || parenEnd === -1 || parenEnd <= parenStart) return
  ...
  private async mineFromDecorators(
    ...
    if (cp.security_contract && cp.security_contract !== "none") {
      const parts = cp.security_contract
        .split(";")
        .map((s) => s.trim())
        .filter(Boolean)
   ...
   */
  private mineFromDecoratorString(decorator: string, symbolId: string, out: InvariantCandidate[]): void {
  ...
  private async mineGlobalAuthPattern(
    ...
    const routeSymbolIds = new Set((routeResult.rows as { symbol_id: string }[]).map((r) => r.symbol_id))
    ...
    if (authRatio >= 0.7) {
      statements.push({
        ...
        params: [
          ...
          "derived",
          Math.min(0.95, 0.8 + authRatio * 0.15),
          "cross_symbol_auth_pattern",
          snapshotId,
        ],
      })
    }
