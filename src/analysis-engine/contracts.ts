...
import type { ContractHint, ContractProfile, Invariant } from "../types"
...
export class ContractEngine {
  ...
  public async extractContractProfile(
    ...
    const inputContract = hint.input_types.length > 0 ? `(${hint.input_types.join(", ")})` : "void"
    const outputContract = hint.output_type || "void"
    const errorContract = hint.thrown_types.length > 0 ? hint.thrown_types.join(" | ") : "never"

    // Extract security and serialization contracts from decorators
    const decorators = hint.decorators || []
    const securityDecorators = decorators.filter((d) => /auth|guard|role|permission|security|token|session/i.test(d))
    const serializationDecorators = decorators.filter((d) =>
      /serialize|transform|expose|exclude|json|xml|proto/i.test(d),
    )
    const schemaDecorators = decorators.filter((d) => /schema|validate|is|matches|min|max|length/i.test(d))
    const apiDecorators = decorators.filter((d) => /get|post|put|delete|patch|route|api|endpoint|controller/i.test(d))
  ...
  public async mineInvariantsFromTests(
    ...
    const testSymbols = symbolVersionRows.filter(
      (sv) =>
        sv.kind === "test_case" ||
    ...
    const schemaSymbols = symbolVersionRows.filter((sv) => sv.kind === "validator" || sv.kind === "schema_object")
    ...
    const assertSymbols = symbolVersionRows.filter((sv) => sv.kind === "function" || sv.kind === "method")
    ...
    const bpMap = new Map<
      string,
      { validation_operations: string[]; exception_profile: string[]; purity_class: string; resource_touches: string[] }
    >()
   ...
   */
  public async buildTestLinkageGraph(snapshotId: string): Promise<Map<string, string[]>> {
