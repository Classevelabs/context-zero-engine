...
export class UncertaintyTracker {
  ...
  public async getSnapshotUncertainty(snapshotId: string): Promise<{
    ...
    const totalSymbols = await db.query(`SELECT COUNT(*) as cnt FROM symbol_versions WHERE snapshot_id = $1`, [
      snapshotId,
    ])
    ...
    if (total > 0 && profiled < total * 0.5) {
      ...
      allAnnotations.push({
        ...
        confidence_impact: Math.min(0.5, uncoveredFraction * 0.5),
