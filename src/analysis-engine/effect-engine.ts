...
const FRAMEWORK_BEHAVIOR_MAP: FrameworkPattern[] = [
  ...
  {
    pattern: /\.(save|insert|create|upsert|bulkCreate|insertMany)\b/i,
    effects: [{ kind: "writes", descriptor: "db.write", detail: "ORM write operation" }],
  },
  {
    pattern: /\.(update|updateOne|updateMany|increment|decrement)\b/i,
    effects: [{ kind: "writes", descriptor: "db.update", detail: "ORM update operation" }],
  },
  {
    pattern: /\.(delete|destroy|remove|deleteOne|deleteMany|truncate)\b/i,
    effects: [{ kind: "writes", descriptor: "db.delete", detail: "ORM delete operation" }],
  },
  {
    pattern: /\.(transaction|beginTransaction|startTransaction)\b/i,
    effects: [
      { kind: "writes", descriptor: "db.transaction", detail: "Database transaction" },
      { kind: "acquires_lock", descriptor: "db.transaction_lock", detail: "Transaction lock acquired" },
    ],
  },
  ...
  {
    pattern:
      /\b(axios|http|https|fetch|got|superagent|request|client|api|restClient|httpClient)\.(get|post|put|patch|delete)\s*\(/i,
    effects: [{ kind: "calls_external", descriptor: "network.http", detail: "HTTP method call" }],
  },
  {
    pattern: /\b(requests)\.(get|post|put|patch|delete|head|options)\s*\(/i,
    effects: [{ kind: "calls_external", descriptor: "network.http", detail: "Python requests HTTP call" }],
    languages: ["python"],
  },
  {
    pattern: /\b(HttpClient|RestTemplate|OkHttpClient|WebClient)\.(get|post|put|patch|delete)\s*\(/i,
    effects: [{ kind: "calls_external", descriptor: "network.http", detail: "HTTP client method call" }],
    languages: ["java", "csharp", "kotlin"],
  },
  {
    pattern: /\breqwest::Client\b.*\.(get|post|put|patch|delete)\s*\(/i,
    effects: [{ kind: "calls_external", descriptor: "network.http", detail: "Rust reqwest HTTP call" }],
    languages: ["rust"],
  },

  // Event emission
  {
    pattern: /\.(emit|publish|dispatch|send|broadcast|trigger)\b/i,
    effects: [{ kind: "emits", descriptor: "event.generic", detail: "Event emission" }],
  },
  {
    pattern: /\b(EventEmitter|PubSub|MessageQueue|kafka|rabbitmq|sns|sqs)\b/i,
    effects: [{ kind: "emits", descriptor: "event.messaging", detail: "Message queue interaction" }],
  },

  // File system
  {
    pattern: /\bfs\.(readFile|readFileSync|readdir|readdirSync|stat|statSync|access)\b/i,
    effects: [{ kind: "reads", descriptor: "file.read", detail: "File system read" }],
  },
  {
    pattern: /\bfs\.(writeFile|writeFileSync|appendFile|mkdir|mkdirSync|unlink|rename|copyFile)\b/i,
    effects: [
      { kind: "writes", descriptor: "file.write", detail: "File system write" },
      { kind: "opens", descriptor: "file.path", detail: "File opened for writing" },
    ],
  },
  {
    pattern: /\bcreateReadStream\b/i,
    effects: [{ kind: "opens", descriptor: "file.stream.read", detail: "Read stream opened" }],
  },
  {
    pattern: /\bcreateWriteStream\b/i,
    effects: [
      { kind: "opens", descriptor: "file.stream.write", detail: "Write stream opened" },
      { kind: "writes", descriptor: "file.stream", detail: "Write stream" },
    ],
  },

  // Cache
  {
    pattern: /\bredis\.(get|mget|hget|hgetall|lrange|smembers)\b/i,
    effects: [{ kind: "reads", descriptor: "cache.redis.read", detail: "Redis read" }],
  },
  {
    pattern: /\bredis\.(set|mset|hset|lpush|rpush|sadd|del|expire|incr|decr)\b/i,
    effects: [{ kind: "writes", descriptor: "cache.redis.write", detail: "Redis write" }],
  },

  // Logging
  {
    pattern: /\b(console|logger|log)\.(log|info|warn|error|debug|trace|fatal)\b/i,
    effects: [{ kind: "logs", descriptor: "log.generic", detail: "Logging call" }],
  },

  // Auth
  {
    pattern: /\b(authenticate|authorize|verifyToken|checkPermission|requireRole|guardRoute)\b/i,
    effects: [{ kind: "requires", descriptor: "auth.check", detail: "Authentication/authorization check" }],
  },
  {
    pattern: /\b(jwt|passport|session)\.(verify|sign|authenticate|decode)\b/i,
    effects: [{ kind: "requires", descriptor: "auth.token", detail: "Token verification" }],
  },

  // Locking / concurrency
  {
    pattern: /\b(mutex|semaphore|lock|acquire|acquireLock)\b/i,
    effects: [{ kind: "acquires_lock", descriptor: "concurrency.lock", detail: "Lock acquisition" }],
  },

  // Validation / normalization
  {
    pattern: /\b(validate|sanitize|normalize|trim|toLowerCase|toUpperCase|slugify)\b/i,
  ...
  // ── Rust-specific ──
  {
    pattern: /\btokio::(spawn|block_on|select!)\b/,
    languages: ["rust"],
    effects: [{ kind: "calls_external", descriptor: "runtime.tokio", detail: "Tokio async runtime" }],
  },
  {
    pattern: /\breqwest::(get|Client)\b/,
    languages: ["rust"],
    effects: [{ kind: "calls_external", descriptor: "network.reqwest", detail: "Reqwest HTTP client" }],
  },
  {
    pattern: /\bstd::fs::/,
    languages: ["rust"],
    effects: [{ kind: "opens", descriptor: "file.rust_fs", detail: "Rust std::fs file operation" }],
  },
  {
    pattern: /\bFile::(open|create)\b/,
    languages: ["rust"],
    effects: [{ kind: "opens", descriptor: "file.rust_file", detail: "Rust File open/create" }],
  },
  {
    pattern: /\b(diesel|sqlx|sea_orm)::.*(select|find|load|get)\b/i,
    languages: ["rust"],
    effects: [{ kind: "reads", descriptor: "db.rust_orm_read", detail: "Rust ORM read" }],
  },
  {
    pattern: /\b(diesel|sqlx|sea_orm)::.*(insert|update|delete|execute)\b/i,
    languages: ["rust"],
    effects: [{ kind: "writes", descriptor: "db.rust_orm_write", detail: "Rust ORM write" }],
  },
  {
    pattern: /\bArc::(new|clone)\b/,
    languages: ["rust"],
    effects: [{ kind: "mutates", descriptor: "state.arc_shared", detail: "Arc shared state" }],
  },
  {
    pattern: /\bMutex::(new|lock)\b/,
    languages: ["rust"],
    effects: [{ kind: "acquires_lock", descriptor: "concurrency.mutex", detail: "Mutex lock" }],
  },
  {
    pattern: /\bRwLock::(new|read|write)\b/,
    languages: ["rust"],
    effects: [{ kind: "acquires_lock", descriptor: "concurrency.rwlock", detail: "RwLock" }],
  },
  {
    pattern: /\bunsafe\s*\{/,
    languages: ["rust"],
    effects: [{ kind: "mutates", descriptor: "state.unsafe_block", detail: "Unsafe memory operations" }],
  },
  {
    pattern: /\bpanic!\b/,
    languages: ["rust"],
    effects: [{ kind: "throws", descriptor: "error.panic", detail: "Rust panic" }],
  },
  {
    pattern: /\b\.unwrap\(\)/,
    languages: ["rust"],
    effects: [{ kind: "throws", descriptor: "error.unwrap_panic", detail: "Potential panic on unwrap" }],
  },
  {
    pattern: /\bserde(_json)?::(to_string|from_str|serialize|deserialize)\b/,
    languages: ["rust"],
    effects: [{ kind: "normalizes", descriptor: "data.serde", detail: "Serde serialization/deserialization" }],
  },
  {
    pattern: /\btracing::(info|warn|error|debug|trace)!?\b/,
    languages: ["rust"],
    effects: [{ kind: "logs", descriptor: "log.tracing", detail: "Tracing log" }],
  },
  {
    pattern: /\blog::(info|warn|error|debug|trace)!?\b/,
    languages: ["rust"],
    effects: [{ kind: "logs", descriptor: "log.rust_log", detail: "Rust log crate" }],
  },

  // ── Java-specific ──
  {
    pattern: /\b(JdbcTemplate|NamedParameterJdbcTemplate)\.(query|queryForObject|queryForList)\b/,
    languages: ["java"],
    effects: [{ kind: "reads", descriptor: "db.jdbc_read", detail: "JDBC query" }],
  },
  {
    pattern: /\b(JdbcTemplate|NamedParameterJdbcTemplate)\.(update|batchUpdate|execute)\b/,
    languages: ["java"],
    effects: [{ kind: "writes", descriptor: "db.jdbc_write", detail: "JDBC update" }],
  },
  {
    pattern: /\bEntityManager\.(find|createQuery|createNamedQuery|getReference)\b/,
    languages: ["java"],
    effects: [{ kind: "reads", descriptor: "db.jpa_read", detail: "JPA read" }],
  },
  {
    pattern: /\bEntityManager\.(persist|merge|remove|flush)\b/,
    languages: ["java"],
    effects: [{ kind: "writes", descriptor: "db.jpa_write", detail: "JPA write" }],
  },
  {
    pattern: /\b(HttpClient|OkHttpClient|RestTemplate|WebClient)\b/,
    languages: ["java"],
    effects: [{ kind: "calls_external", descriptor: "network.java_http", detail: "Java HTTP client" }],
  },
  {
    pattern: /\bFiles\.(read|write|copy|move|delete|walk|list|newInputStream|newOutputStream)\b/,
    languages: ["java"],
    effects: [{ kind: "opens", descriptor: "file.java_nio", detail: "Java NIO file operation" }],
  },
  {
    pattern: /\b(FileInputStream|FileOutputStream|BufferedReader|BufferedWriter)\b/,
    languages: ["java"],
    effects: [{ kind: "opens", descriptor: "file.java_io", detail: "Java IO file operation" }],
  },
  {
    pattern: /\bsynchronized\b/,
    languages: ["java"],
    effects: [
      { kind: "acquires_lock", descriptor: "concurrency.java_synchronized", detail: "Java synchronized block" },
    ],
  },
  {
    pattern: /\b(ReentrantLock|StampedLock|CountDownLatch|Semaphore)\b/,
    languages: ["java"],
    effects: [{ kind: "acquires_lock", descriptor: "concurrency.java_lock", detail: "Java concurrency lock" }],
  },
  {
    pattern: /\b@Transactional\b/,
    languages: ["java"],
    effects: [
      { kind: "writes", descriptor: "db.transaction", detail: "Spring @Transactional" },
      { kind: "acquires_lock", descriptor: "db.transaction_lock", detail: "Transaction lock" },
    ],
  },
  {
    pattern: /\bLogger\.(info|warn|error|debug|trace)\b/,
    languages: ["java"],
    effects: [{ kind: "logs", descriptor: "log.java_logger", detail: "Java Logger" }],
  },

  // ── C#-specific ──
  {
    pattern: /\bDbContext\b/,
    languages: ["csharp"],
    effects: [{ kind: "reads", descriptor: "db.ef_context", detail: "Entity Framework DbContext" }],
  },
  {
    pattern: /\b(DbSet|IQueryable)\.(Where|Select|Include|FirstOrDefault|ToList|Any|Count)\b/,
    languages: ["csharp"],
    effects: [{ kind: "reads", descriptor: "db.ef_read", detail: "EF LINQ query" }],
  },
  {
    pattern: /\b(SaveChanges|SaveChangesAsync|Add|Remove|Update)\b/,
    languages: ["csharp"],
    effects: [{ kind: "writes", descriptor: "db.ef_write", detail: "EF write operation" }],
  },
  {
    pattern: /\bHttpClient\.(GetAsync|PostAsync|PutAsync|DeleteAsync|SendAsync)\b/,
    languages: ["csharp"],
    effects: [{ kind: "calls_external", descriptor: "network.csharp_http", detail: "C# HttpClient" }],
  },
  {
    pattern: /\b(File|Directory)\.(Read|Write|Create|Delete|Move|Copy|Exists)\b/,
    languages: ["csharp"],
    effects: [{ kind: "opens", descriptor: "file.csharp_io", detail: "C# file I/O" }],
  },
  {
    pattern: /\block\s*\(/,
    languages: ["csharp"],
    effects: [{ kind: "acquires_lock", descriptor: "concurrency.csharp_lock", detail: "C# lock statement" }],
  },
  {
    pattern: /\b(SemaphoreSlim|Monitor|Mutex)\b/,
    languages: ["csharp"],
    effects: [{ kind: "acquires_lock", descriptor: "concurrency.csharp_sync", detail: "C# synchronization primitive" }],
  },
  {
    pattern: /\bILogger\.(Log|Info|Warn|Error|Debug|Trace)\b/i,
    languages: ["csharp"],
    effects: [{ kind: "logs", descriptor: "log.csharp_ilogger", detail: "C# ILogger" }],
  },

  // ── Ruby-specific ──
  {
    pattern: /\b(ActiveRecord|ApplicationRecord)/,
    languages: ["ruby"],
    effects: [{ kind: "reads", descriptor: "db.activerecord", detail: "ActiveRecord model" }],
  },
  {
    pattern: /\.(where|find|find_by|pluck|select|joins|includes|eager_load)\b/,
    languages: ["ruby"],
    effects: [{ kind: "reads", descriptor: "db.ar_read", detail: "ActiveRecord read" }],
  },
  {
    pattern: /\.(create|update|destroy|save|delete|insert)\b/,
    languages: ["ruby"],
    effects: [{ kind: "writes", descriptor: "db.ar_write", detail: "ActiveRecord write" }],
  },
  {
    pattern: /\b(Net::HTTP|HTTParty|Faraday|RestClient)\b/,
    languages: ["ruby"],
    effects: [{ kind: "calls_external", descriptor: "network.ruby_http", detail: "Ruby HTTP client" }],
  },
  {
    pattern: /\bFile\.(open|read|write|delete)\b/,
    languages: ["ruby"],
    effects: [{ kind: "opens", descriptor: "file.ruby_io", detail: "Ruby File I/O" }],
  },
  {
    pattern: /\bRails\.cache\b/,
    languages: ["ruby"],
    effects: [{ kind: "reads", descriptor: "cache.rails", detail: "Rails cache" }],
  },
  {
    pattern: /\bRedis\b/,
    languages: ["ruby"],
    effects: [{ kind: "reads", descriptor: "cache.redis", detail: "Redis client" }],
  },
  {
    pattern: /\braise\s+\w/,
    languages: ["ruby"],
    effects: [{ kind: "throws", descriptor: "error.ruby_raise", detail: "Ruby exception raised" }],
  },
  {
    pattern: /\bRails\.logger\b/,
    languages: ["ruby"],
    effects: [{ kind: "logs", descriptor: "log.rails", detail: "Rails logger" }],
  },

  // ── Go-specific ──
  {
    pattern: /\bhttp\.(Get|Post|Head|NewRequest)\b/,
    languages: ["go"],
    effects: [{ kind: "calls_external", descriptor: "network.go_http", detail: "Go HTTP client" }],
  },
  {
    pattern: /\bsql\.(Open|DB)\b/,
    languages: ["go"],
    effects: [{ kind: "reads", descriptor: "db.go_sql", detail: "Go database/sql" }],
  },
  {
    pattern: /\bos\.(Open|Create|Remove|Mkdir)\b/,
    languages: ["go"],
    effects: [{ kind: "opens", descriptor: "file.go_os", detail: "Go os file operation" }],
  },
  {
    pattern: /\bsync\.(Mutex|RWMutex|WaitGroup)\b/,
    languages: ["go"],
    effects: [{ kind: "acquires_lock", descriptor: "concurrency.go_sync", detail: "Go sync primitive" }],
  },
  {
    pattern: /\bgo\s+\w+\(/,
    languages: ["go"],
    effects: [{ kind: "calls_external", descriptor: "runtime.goroutine", detail: "Goroutine launched" }],
  },
  {
    pattern: /\b(log|slog)\.(Print|Fatal|Panic|Info|Warn|Error|Debug)\b/,
    languages: ["go"],
    effects: [{ kind: "logs", descriptor: "log.go", detail: "Go logger" }],
  },

  // ── C++-specific ──
  {
    pattern: /\bstd::thread\b/,
    languages: ["cpp"],
    effects: [{ kind: "calls_external", descriptor: "runtime.cpp_thread", detail: "C++ thread" }],
  },
  {
    pattern: /\bstd::(mutex|shared_mutex|lock_guard|unique_lock)\b/,
    languages: ["cpp"],
    effects: [{ kind: "acquires_lock", descriptor: "concurrency.cpp_mutex", detail: "C++ mutex" }],
  },
  {
    pattern: /\bstd::(ifstream|ofstream|fstream)\b/,
    languages: ["cpp"],
    effects: [{ kind: "opens", descriptor: "file.cpp_fstream", detail: "C++ file stream" }],
  },
  {
    pattern: /\bboost::asio\b/,
    languages: ["cpp"],
    effects: [{ kind: "calls_external", descriptor: "network.boost_asio", detail: "Boost.Asio network" }],
  },
  {
    pattern: /\bstd::cout\b/,
    languages: ["cpp"],
    effects: [{ kind: "logs", descriptor: "log.cpp_cout", detail: "C++ stdout" }],
  },
  {
    pattern: /\bstd::cerr\b/,
    languages: ["cpp"],
    effects: [{ kind: "logs", descriptor: "log.cpp_cerr", detail: "C++ stderr" }],
  },
]
...
export class EffectEngine {
   ...
   */
  public async computeForSymbol(symbolVersionId: string, behaviorHints: BehaviorHint[]): Promise<EffectSignature> {
    ...
    const sv = svResult.rows[0] as
      | { body_source: string | null; signature: string; summary: string; language: string; kind: string }
      | undefined
    if (sv) {
      // Skip framework pattern mining for class/interface/type_alias symbols
      const isContainerKind = sv.kind === "class" || sv.kind === "interface" || sv.kind === "type_alias"
    ...
    const confidence = this.computeConfidence(
      bp !== undefined,
      cp !== undefined,
      sv?.body_source !== null && sv?.body_source !== undefined,
    )
  ...
  public async propagateEffectsTransitive(snapshotId: string): Promise<number> {
    ...
    for (const [svId, sig] of signatures) {
      effectKeySets.set(svId, new Set(sig.effects.map((e) => `${e.kind}:${e.descriptor}`)))
    }
  ...
  public async diffEffects(beforeSvId: string, afterSvId: string): Promise<EffectDiff> {
    const [before, after] = await Promise.all([this.getEffectSignature(beforeSvId), this.getEffectSignature(afterSvId)])
    ...
    const classDirection: "escalated" | "deescalated" | "unchanged" =
      afterLevel > beforeLevel ? "escalated" : afterLevel < beforeLevel ? "deescalated" : "unchanged"
  ...
  public classifyEffectClass(effects: EffectEntry[]): EffectClass {
    ...
    const hasTransaction = effects.some((e) => e.kind === "writes" && e.descriptor.includes("transaction"))
    if (hasTransaction) {
      return "full_side_effect"
    }
  ...
  private mineFromContractProfile(cp: ContractProfile): EffectEntry[] {
    ...
    if (cp.security_contract && cp.security_contract !== "none") {
      ...
      const secParts = cp.security_contract
        .split(";")
        .map((s) => s.trim())
        .filter(Boolean)
    ...
    if (cp.error_contract && cp.error_contract !== "never") {
      ...
      const errorParts = cp.error_contract
        .split("|")
        .map((s) => s.trim())
        .filter(Boolean)
  ...
  private collectDescriptors(effects: EffectEntry[], kind: EffectKind): string[] {
    return [...new Set(effects.filter((e) => e.kind === kind).map((e) => e.descriptor))]
  }
   ...
   */
  private computeConfidence(hasBehavioral: boolean, hasContract: boolean, hasBodySource: boolean): number {
    let confidence = 0.5 // base confidence for framework pattern matching alone

    if (hasBehavioral) confidence += 0.2
    if (hasContract) confidence += 0.15
    if (hasBodySource) confidence += 0.1
