...
import type { TracePack, TraceCallEdge, TraceDynamicRoute, TraceObservedType, TraceFrameworkEvent } from "../types"
...
export class RuntimeEvidenceEngine {
   ...
   */
  async ingestTrace(repoId: string, snapshotId: string, tracePack: TracePack): Promise<TraceIngestionResultExtended> {
    ...
    const traceTimestamp =
      tracePack.timestamp instanceof Date && !isNaN(tracePack.timestamp.getTime()) ? tracePack.timestamp : new Date()
  ...
  private validateTracePack(tracePack: TracePack): string[] {
    ...
    if (!VALID_TRACE_SOURCES.has(tracePack.source)) {
      errors.push(
        `Invalid trace source: '${tracePack.source}'. Valid sources: test_execution, dev_run, ci_trace, production_sample`,
      )
    }
    ...
    if (!Array.isArray(tracePack.call_edges)) {
      ...
      if (tracePack.call_edges.length > MAX_CALL_EDGES_PER_TRACE) {
        errors.push(
          `call_edges exceeds maximum of ${MAX_CALL_EDGES_PER_TRACE} (got ${tracePack.call_edges.length}), will be truncated`,
        )
      }
    }
  ...
  private async buildSymbolLookupIndex(snapshotId: string): Promise<SymbolLookupIndex> {
    ...
    for (const sv of svRows) {
      ...
      if (sv.file_path && sv.range_start_line) {
        byFileLine.set(`${this.normalizePath(sv.file_path)}:${sv.range_start_line}`, sv.symbol_version_id)
      }
      ...
      if (sv.file_path && sv.canonical_name) {
        ...
        byFileName.set(`${this.normalizePath(sv.file_path)}::${simpleName}`, sv.symbol_version_id)
      }
    }
   ...
   */
  async resolveFrame(_repoId: string, snapshotId: string, frame: RuntimeFrame): Promise<string | null> {
    const lookupIndex = await this.buildSymbolLookupIndex(snapshotId)
   ...
   */
  private resolveFrameWithIndex(frame: RuntimeFrame, lookupIndex: SymbolLookupIndex): string | null {
    ...
    if (frame.file_path && frame.line_number) {
      ...
      const byFileLine = lookupIndex.byFileLine.get(`${normalizedPath}:${frame.line_number}`)
      if (byFileLine) return byFileLine

      // Try nearby lines (off-by-one from source maps, instrumentation)
      const deltas = [-1, 1, -2, 2]
      for (const delta of deltas) {
        const nearby = lookupIndex.byFileLine.get(`${normalizedPath}:${frame.line_number + delta}`)
    ...
    if (frame.file_path) {
      ...
      const byFileName = lookupIndex.byFileName.get(`${normalizedPath}::${simpleName}`)
   ...
   */
  private resolveKeyToSymbolVersion(key: string, lookupIndex: SymbolLookupIndex): string | null {
    if (!key || typeof key !== "string") return null

    // Direct lookup first (covers stable_key and canonical_name)
    const direct = lookupIndex.byStableKey.get(key) || lookupIndex.byCanonical.get(key)
  ...
  async mergeObservedEdges(snapshotId: string): Promise<number> {
    ...
    for (const row of staticEdgesResult.rows as {
      relation_id: string
    ...
    for (const row of observedResult.rows as {
      ...
      if (existing) {
        const boostedConfidence = Math.min(CONFIDENCE_CAP, existing.confidence + RUNTIME_CONFIRMATION_BOOST)
        ...
        statements.push({
          ...
          params: [uuidv4(), callerSvId, calleeSvId, 1.0, observationConfidence],
        })
        affected++
      }
      ...
      if (row.receiver_type) {
        statements.push({
          ...
          params: [observationConfidence, row.receiver_type, calleeSvId, callerSvId, snapshotId],
        })
      }
    }
  ...
  async reduceUncertainty(snapshotId: string): Promise<number> {
    ...
    for (const row of unresolvedDispatch.rows as {
      ...
      if (runtimeTarget.rowCount && runtimeTarget.rowCount > 0) {
        ...
        const receiverTypes = targets.map((t) => t.receiver_type).filter((t): t is string => t !== null)
        const totalCalls = targets.reduce((sum, t) => sum + parseInt(t.total_calls, 10), 0)
        ...
        statements.push({
          ...
          params: [resolvedIds, receiverTypes, confidence, resolvedIds.length > 1, row.dispatch_edge_id],
        })
        reduced++
      }
    }
   ...
   */
  private async processObservedTypes(snapshotId: string, observedTypes: TraceObservedType[]): Promise<void> {
    ...
    for (const obs of observedTypes) {
      ...
      statements.push({
        ...
        params: [obs.observed_type, FILE_LINE_MATCH_CONFIDENCE, snapshotId, obs.expression],
      })
    }
    ...
    for (const event of events) {
      ...
      switch (event.event_type) {
        ...
        case "middleware_mount": {
          ...
          const middlewareKey =
            typeof detailObj["middleware_key"] === "string" ? detailObj["middleware_key"] : undefined
  ...
  async getEvidenceForSymbol(symbolVersionId: string): Promise<RuntimeEvidenceExtended> {
    ...
    const observedAsCaller: ObservedEdgeSummary[] = (
      callerEdges.rows as {
    ...
    const observedAsCallee: ObservedEdgeSummary[] = (
      calleeEdges.rows as {
    ...
    for (const e of observedAsCaller) {
      sourceSet.add(e.trace_source)
    }
    for (const e of observedAsCallee) {
      sourceSet.add(e.trace_source)
    }
    ...
    const confidenceBoost = Math.min(
      0.3,
      totalObservations > 0 ? RUNTIME_CONFIRMATION_BOOST * Math.min(sourceDiversity, 3) : 0,
    )
  ...
  private demangleName(name: string): string {
    ...
    demangled = demangled.replace(
      /^__(?:awaiter|generator|decorate|param|metadata|asyncGenerator|asyncDelegator|asyncValues|rest|spread|spreadArrays)$/,
      "",
    )
   ...
   */
  private computeEdgeConfidence(callerSvId: string | null, calleeSvId: string | null, callCount: number): number {
    let base: number
    ...
    const countBonus = Math.min(0.1, Math.log10(Math.max(1, callCount)) * 0.03)
