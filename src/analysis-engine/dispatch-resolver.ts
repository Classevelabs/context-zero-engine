...
// Every value dispatch_edges.resolution_method is allowed to hold — the RESOLUTION enum plus
// the point-to fact sources that flow through `segmentMethod = fact.source` (an otherwise
// unbounded string). The chk_de_resolution_method CHECK constraint must list exactly these.
// Kept adjacent to RESOLUTION so the constraint migration and the code can never drift.
export const ALLOWED_RESOLUTION_METHODS: ReadonlySet<string> = new Set(Object.values(RESOLUTION))

/** Coerce any resolution_method to a value the DB constraint accepts. `fact.source` can be a
 *  points-to provenance tag outside the enum (or a future value); rather than fail the whole
 *  batch insert on one unknown method — the bug that silently dropped EVERY dispatch edge for a
 *  snapshot — record the edge with a safe 'unresolved' method. The edge itself (caller,
 *  receiver, resolved targets) is the valuable part; the method is only provenance. */
export function normalizeResolutionMethod(method: string): string {
  return ALLOWED_RESOLUTION_METHODS.has(method) ? method : RESOLUTION.UNRESOLVED
}
...
export class DispatchResolver {
   ...
   */
  private static svByCanonicalFirst(map: Map<string, SymbolVersionRow[]>, name: string): SymbolVersionRow | undefined {
  ...
  async buildClassHierarchy(snapshotId: string): Promise<number> {
    ...
    for (const [svId, node] of classGraph) {
      ...
      for (let position = 0; position < mro.length; position++) {
        ...
        const relationKind =
          position === 0 ? "self" : (node.parents.find((p) => p.svId === ancestorSvId)?.relationKind ?? "extends")
        ...
        statements.push({
          ...
          params: [uuidv4(), snapshotId, svId, position === 0 ? null : ancestorSvId, position, relationKind],
        })
        insertedCount++
      }
    }
   ...
   */
  private computeC3Linearization(classSvId: string, graph: Map<string, ClassNode>): string[] {
  ...
  private c3Linearize(
    ...
    const mergeInputs = [...parentLinearizations.map((l) => [...l]), [...parentSvIds]]
   ...
   */
  private dfsMRO(svId: string, graph: Map<string, ClassNode>, visited: Set<string>): string[] {
      ...
      for (const line of lines) {
        ...
        if (tsFieldMatch && tsFieldMatch[1] && tsFieldMatch[2]) {
          ...
          for (const t of types) {
            this.mergePointsToFact(ownerFacts, ownerSvId, fieldName, t, "field_annotation", 0.9, svByCanonical)
          }
          continue
        }
        ...
        if (
          ...
          (trimmed.includes("constructor") ||
            sv.canonical_name.endsWith(".constructor") ||
            sv.canonical_name.endsWith(".__init__"))
        ) {
          const fieldName = diMatch[1]
          const rawType = diMatch[2].trim()
          const types = this.parseUnionType(rawType)
          for (const t of types) {
            this.mergePointsToFact(ownerFacts, ownerSvId, fieldName, t, "dependency_injection", 0.92, svByCanonical)
          }
          continue
        }
        ...
        if (dataclassMatch && dataclassMatch[1] && dataclassMatch[2]) {
          ...
          const types = this.parseUnionType(rawType)
          for (const t of types) {
            this.mergePointsToFact(ownerFacts, ownerSvId, fieldName, t, "dataclass_attr", 0.93, svByCanonical)
          }
          continue
        }
        ...
        if (pyFieldMatch && pyFieldMatch[1] && pyFieldMatch[2] && trimmed.includes("self.")) {
          ...
          const types = this.parseUnionType(rawType)
          for (const t of types) {
            this.mergePointsToFact(ownerFacts, ownerSvId, fieldName, t, "field_annotation", 0.88, svByCanonical)
          }
          continue
        }
        ...
        if (rustFieldMatch && rustFieldMatch[1] && rustFieldMatch[2] && sv.language === "rust") {
          ...
          const innerType = rawType
            .replace(/^(?:Box|Arc|Rc|Mutex|RwLock|Option|Vec|HashMap|BTreeMap|Result)\s*<\s*/, "")
            .replace(/\s*>$/, "")
            .split(",")[0]
            ?.trim()
          if (innerType) {
            this.mergePointsToFact(ownerFacts, ownerSvId, fieldName, innerType, "field_annotation", 0.9, svByCanonical)
          }
          continue
        }

        // 7. Java/C# field: private TypeName fieldName;
        const javaFieldMatch = trimmed.match(
          /^(?:private|protected|public|internal|readonly|final|static)?\s*(?:private|protected|public|internal|readonly|final|static)?\s*(\w[\w<>,.\s]*?)\s+(\w+)\s*[;=]/,
        )
        if (
          javaFieldMatch &&
          javaFieldMatch[1] &&
          javaFieldMatch[2] &&
          (sv.language === "java" || sv.language === "csharp")
        ) {
          ...
          if (
            ...
            this.mergePointsToFact(ownerFacts, ownerSvId, fieldName, rawType, "field_annotation", 0.9, svByCanonical)
          }
          continue
        }
        ...
        if (goFieldMatch && goFieldMatch[1] && goFieldMatch[2] && sv.language === "go") {
          ...
          this.mergePointsToFact(ownerFacts, ownerSvId, fieldName, rawType, "field_annotation", 0.88, svByCanonical)
          continue
        }
   ...
   */
  async resolveChain(snapshotId: string, callerSvId: string, chain: string): Promise<DispatchResolution> {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      for (const currentType of currentTypes) {
        ...
        if (isLast) {
          ...
          if (methodSv) {
            ...
            segmentConfidence = Math.max(segmentConfidence, 0.9)
          }
        ...
        if (segmentTypes.length === 0) {
          ...
          if (nestedSv) {
            ...
            segmentConfidence = Math.max(segmentConfidence, 0.7)
          }
        }
      }
      ...
      // Java/C# (this.).
      // -----------------------------------------------------------------
      if (segmentTypes.length === 0 && segmentSvIds.length === 0 && ownerCanonical && segments.length === 1) {
        ...
        const bareSv = !qualifiedSv
          ? DispatchResolver.svByCanonicalInFile(svByCanonical, segment, callerFileId)
          : undefined
        ...
        if (resolvedSameClassSv) {
          ...
          segmentMethod = RESOLUTION.FIELD_INFERENCE
          segmentConfidence = qualifiedSv ? 0.85 : 0.8
  ...
  async resolveDispatches(snapshotId: string, repoId: string): Promise<number> {
    ...
    for (const callsite of callsites) {
      ...
      if (
        resolution.resolved_symbol_version_ids.length > 0 &&
      ...
      statements.push({
        ...
        params: [
          ...
          normalizeResolutionMethod(resolution.resolution_method),
  ...
  async getMRO(snapshotId: string, classSvId: string): Promise<string[]> {
    ...
    for (const row of relResult.rows as {
      src_symbol_version_id: string
      dst_symbol_version_id: string
