...
export class SymbolLineageEngine {
  ...
  public async computeLineage(
    ...
    if (previousSnapshotId && unmatchedSymbols.length > 0) {
      ...
      const unmatchedLineages = aliveLineages.filter((l) => !matchedLineageIds.has(l.lineage_id))
      ...
      if (unmatchedLineages.length > 0) {
        ...
        for (const rename of renames) {
          ...
          // Create new lineage linked to old via previous_lineage_id, and link the
          // renamed symbol to whatever id the upsert settles on — same CTE fix as the
          // births path below (ON CONFLICT can return an existing id ≠ the generated
          // one; linking to the generated id violated symbols_lineage_id_fkey).
          ...
          newLineageStatements.push({
            text: `WITH upserted AS (
                               ...
                               UPDATE symbols SET lineage_id = (SELECT lineage_id FROM upserted)
                               WHERE symbol_id = $10`,
            params: (() => {
        ...
        const stillUnmatched = unmatchedSymbols.filter((s) => !renamedSymbolIds.has(s.symbolId))
    ...
    for (const { symbolId, seed, symbol } of unmatchedSymbols) {
      ...
      // Insert-and-link in ONE statement via CTE: the ON CONFLICT (repo_id, identity_seed)
      // can resolve to an EXISTING lineage row whose lineage_id is NOT the `lineageId` we
      // generated here — so linking the symbol to that generated id separately pointed it
      // at a row that never existed, violating symbols_lineage_id_fkey and dropping ALL
      // lineage for the snapshot. RETURNING the id the upsert actually settled on and using
      // THAT for the link is correct whether the row was born or matched.
      newLineageStatements.push({
        text: `WITH upserted AS (
                           INSERT INTO symbol_lineage (
                               lineage_id, repo_id, identity_seed, canonical_name, kind, language,
                       ...
                       UPDATE symbols SET lineage_id = (SELECT lineage_id FROM upserted)
                       WHERE symbol_id = $8`,
        params: [lineageId, repoId, seed, symbol.canonical_name, symbol.kind, symbol.language, snapshotId, symbolId],
      })
    ...
    const allStatements = [...newLineageStatements, ...linkStatements, ...deathStatements]
  ...
  public async matchRenamedSymbols(
    ...
    const unmatchedLineageNames = unmatchedLineages ? new Set(unmatchedLineages.map((l) => l.canonical_name)) : null
  ...
  public async getLineageHistory(symbolId: string): Promise<LineageEntry[]> {
    ...
    const symbolResult = await db.query(`SELECT lineage_id FROM symbols WHERE symbol_id = $1`, [symbolId])
  ...
  public async getLineageById(lineageId: string): Promise<LineageEntry | null> {
    const result = await db.query(`SELECT * FROM symbol_lineage WHERE lineage_id = $1`, [lineageId])
  ...
  public async getLineageStats(repoId: string): Promise<{
    ...
    const [totalResult, aliveResult, renamedResult, byKindResult, byLangResult] = await Promise.all([
      ...
      db.query(`SELECT COUNT(*) as cnt FROM symbol_lineage WHERE repo_id = $1 AND previous_lineage_id IS NOT NULL`, [
        repoId,
      ]),
   ...
   */
  private async buildNeighborhoodMap(snapshotId: string): Promise<Map<string, NeighborhoodSet>> {
   ...
   *   - Different: 0.0
   */
  private computeNormalizedAstSimilarity(oldSym: SnapshotSymbolRow, newSym: SnapshotSymbolRow): number {
    if (
      oldSym.normalized_ast_hash &&
      newSym.normalized_ast_hash &&
   ...
   */
  private computeBodyHashSimilarity(oldSym: SnapshotSymbolRow, newSym: SnapshotSymbolRow): number {
   ...
   */
  private computeNeighborhoodSimilarity(oldNeighbors: NeighborhoodSet, newNeighbors: NeighborhoodSet): number {
    ...
    if (
      oldNeighbors.callers.size === 0 &&
      newNeighbors.callers.size === 0 &&
      oldNeighbors.callees.size === 0 &&
      newNeighbors.callees.size === 0
    ) {
      return 0.0
    }
  ...
  private computeNameEditDistance(nameA: string, nameB: string): number {
    ...
    const distance = this.levenshteinDistance(nameA.toLowerCase(), nameB.toLowerCase())
