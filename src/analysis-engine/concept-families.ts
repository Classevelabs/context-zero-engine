...
const MIN_INTERNAL_DENSITY = 0.4
...
export class ConceptFamilyEngine {
  ...
  public async buildFamilies(repoId: string, snapshotId: string): Promise<FamilyBuildResult> {
    ...
    await db.transaction(async (client) => {
      ...
      await db.queryWithClient(client, `DELETE FROM concept_families WHERE repo_id = $1 AND snapshot_id = $2`, [
        repoId,
        snapshotId,
      ])
    })
    ...
    if (rawClusters.length === 0) {
      log.info("No clusters at primary threshold, retrying at fallback", {
        primary: MIN_EDGE_CONFIDENCE,
        fallback: FALLBACK_EDGE_CONFIDENCE,
      })
    ...
    for (const cluster of rawClusters) {
      ...
      const exemplarSvId = this.selectExemplarSync(activeSvIds, cluster.internal_edges, behavioralMap, contractMap)
   ...
   */
  private extractInternalEdges(nodes: string[], adjacency: Map<string, Map<string, AdjacencyData>>): EdgeRecord[] {
   ...
   */
  private bisectCluster(nodes: string[], adjacency: Map<string, Map<string, AdjacencyData>>): string[][] {
    ...
    for (let pass = 0; pass < MAX_PASSES && improved; pass++) {
      ...
      for (const node of nodes) {
        ...
        const delta = this.computeModularityDelta(node, currentPartition, otherPartition, weightMatrix, degree, m)
  ...
  private computeModularityDelta(
    ...
    const delta = (sumToOther - sumToCurrent) / m + (nodeDeg * (sumDegreesCurrent - sumDegreesOther)) / (2 * m * m)
  ...
  public classifyFamilyType(
    ...
    if (behavioralMap && behavioralMap.size > 0) {
      ...
      for (const svId of memberSvIds) {
        ...
        const hasCatchOrThrow = exceptions.some((e) => e.startsWith("catches:") || e.startsWith("throws:"))
        ...
        const nameBilling = md ? /billing|invoice|payment|charge|subscription|price/i.test(md.canonical_name) : false
  ...
  private computeFamilyAvgSimilarity(
    ...
    const relevantEdges = edges.filter((e) => memberSet.has(e.src) && memberSet.has(e.dst))
   ...
   */
  public async computeFingerprints(familySvIds: string[]): Promise<{ contract: string; effect: string }> {
   ...
   */
  public async detectOutliers(familySvIds: string[], exemplarSvId: string): Promise<OutlierResult[]> {
    if (familySvIds.length === 0) return []
  ...
  private detectOutliersFromCache(
    ...
    for (const svId of familySvIds) {
      ...
      const directEdge = edges.find(
        (e) => (e.src === svId && e.dst === exemplarSvId) || (e.dst === svId && e.src === exemplarSvId),
      )
      ...
      if (directEdge) {
        ...
        const memberEdges = edges.filter((e) => e.src === svId || e.dst === svId)
    ...
    for (const svId of familySvIds) {
      ...
      if (exemplarBp && memberBp) {
        ...
        if (exemplarBp.purity_class !== memberBp.purity_class) {
          contradictionFlags.push(`purity_diverges:${exemplarBp.purity_class}->${memberBp.purity_class}`)
        }
      ...
      if (exemplarCp && memberCp) {
        ...
        if (
          exemplarCp.error_contract !== memberCp.error_contract &&
          exemplarCp.error_contract !== "never" &&
          memberCp.error_contract !== "never"
        ) {
        ...
        if (
          exemplarCp.security_contract !== memberCp.security_contract &&
          exemplarCp.security_contract !== "none" &&
          memberCp.security_contract !== "none"
        ) {
  ...
  private computeMembershipConfidence(svId: string, cluster: RawCluster): number {
    const memberEdges = cluster.internal_edges.filter((e) => e.src === svId || e.dst === svId)
