...
async function bench(mode: "minimal" | "standard" | "strict", target: string, snapshotId: string) {
  ...
  const coChangeHits =
    (capsule as any).nodes?.filter(
      (n: any) =>
        Array.isArray(n.inbound_edges) && n.inbound_edges.some((e: any) => e.relation_type === "co_changed_with"),
    )?.length ?? 0
...
async function main() {
  ...
  for (const mode of ["minimal", "standard", "strict"] as const) {
    ...
    results.push(r)
    console.log(
      `[bench] ${mode.padEnd(8)}  ${r.elapsedMs}ms  nodes=${r.nodeCount}  bytes=${r.byteSize}  ~${r.tokenEstimate} tokens  lineage_covered=${r.lineageHits}  co_change_covered=${r.coChangeHits}`,
    )
  }
