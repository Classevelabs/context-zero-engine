...
const outputPath = path.resolve(process.argv[3] || path.join(contextZeroRoot, 'MULTILANGUAGE_CONTEXTZERO_BENCHMARK_REPORT.md'));
...
const sourceExts = new Set([
    '.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs', '.py', '.pyi', '.pyw',
    '.rs', '.go', '.java', '.cs', '.cpp', '.cc', '.cxx', '.c', '.h', '.hpp',
    '.rb', '.kt', '.kts', '.swift', '.php', '.sh', '.bash', '.json', '.md',
    '.yml', '.yaml',
]);

const skipDirs = new Set([
    '.git', '.svn', '.hg', 'node_modules', 'dist', 'build', '.next', '.nuxt',
    '.turbo', '.yarn', '.pnpm-store', '__pycache__', '.venv', 'venv',
    '.mypy_cache', '.pytest_cache', '.ruff_cache', '.tox', '.eggs', 'egg-info',
    'target', 'vendor', '.gradle', '.mvn', 'bin', 'obj', 'packages', 'Pods',
    '.build', '.idea', '.vscode', 'coverage', '.cache', 'tmp', 'temp',
]);
...
const allTargets: BenchTarget[] = [
    ...
    { id: 'prometheus', name: 'Prometheus', pathName: 'prometheus', primaryLanguages: ['go'], note: 'Go monitoring system' },
    { id: 'tokio', name: 'Tokio', pathName: 'tokio', primaryLanguages: ['rust'], note: 'Rust async runtime' },
    { id: 'spring-framework', name: 'Spring Framework', pathName: 'spring-framework', primaryLanguages: ['java'], note: 'Java application framework' },
    { id: 'commons-lang', name: 'Apache Commons Lang', pathName: 'commons-lang', primaryLanguages: ['java'], note: 'Java utility library' },
    { id: 'dotnet-runtime', name: '.NET Runtime', pathName: 'dotnet-runtime', primaryLanguages: ['csharp'], note: 'C#/.NET runtime repository' },
    { id: 'serilog', name: 'Serilog', pathName: 'serilog', primaryLanguages: ['csharp'], note: 'C# structured logging library' },
    { id: 'rails', name: 'Rails', pathName: 'rails', primaryLanguages: ['ruby'], note: 'Ruby web framework' },
    { id: 'okhttp', name: 'OkHttp', pathName: 'okhttp', primaryLanguages: ['kotlin', 'java'], note: 'Kotlin/Java HTTP client' },
    { id: 'alamofire', name: 'Alamofire', pathName: 'alamofire', primaryLanguages: ['swift'], note: 'Swift networking library' },
    { id: 'bitcoin', name: 'Bitcoin Core', pathName: 'bitcoin', primaryLanguages: ['cpp'], note: 'C++ Bitcoin node' },
    { id: 'laravel-framework', name: 'Laravel Framework', pathName: 'laravel-framework', primaryLanguages: ['php'], note: 'PHP web framework' },
...
async function collectDbCounts(snapshotId: string): Promise<Record<string, number>> {
    return {
        ...
        symbol_versions: await countRows('SELECT COUNT(*)::int AS count FROM symbol_versions WHERE snapshot_id = $1', [snapshotId]),
        ...
        invariants: await countRows('SELECT COUNT(*)::int AS count FROM invariants WHERE last_verified_snapshot_id = $1', [snapshotId]),
    };
}
...
async function selectTargets(snapshotId: string, languages: string[]): Promise<Array<{
...
async function exactSymbolFootprint(root: string, symbol: string): Promise<{ files: number; bytes: number; elapsed_ms: number }> {
...
async function runTimed(name: string, fn: () => Promise<unknown>): Promise<{ name: string; elapsed_ms: number; bytes: number; estimated_tokens: number }> {
    const start = performance.now();
    const data = await fn();
...
async function tokenBenchmarks(root: string, snapshotId: string, languages: string[], corpus: CorpusStats): Promise<{
    rows: TokenTask[];
    ...
    if (firstTarget) {
        queryMetrics.push(await runTimed('capsule_strict_first_target', () =>
            capsuleCompiler.compile(firstTarget.symbol_version_id, snapshotId, 'strict'),
        ));
        queryMetrics.push(await runTimed('blast_radius_depth_1_first_target', () =>
...
async function adapterSmoke(): Promise<SmokeRow[]> {
    ...
    try {
        ...
        rows.push({ language: 'typescript/javascript', adapter: 'TypeScript Compiler API', status: r.symbols.length > 0 ? 'pass' : 'fail', symbols: r.symbols.length, relations: r.relations.length, note: '' });
    } catch (error) {
        rows.push({ language: 'typescript/javascript', adapter: 'TypeScript Compiler API', status: 'fail', symbols: 0, relations: 0, note: error instanceof Error ? error.message : String(error) });
    }
    ...
    try {
        ...
        rows.push({ language: 'python', adapter: 'LibCST subprocess', status: py.status === 0 && (parsed.symbols?.length || 0) > 0 ? 'pass' : 'fail', symbols: parsed.symbols?.length || 0, relations: parsed.relations?.length || 0, note: py.stderr.trim() });
    } catch {
        rows.push({ language: 'python', adapter: 'LibCST subprocess', status: 'fail', symbols: 0, relations: 0, note: py.stderr.trim() || py.stdout.trim() });
    }
    ...
    const samples: Array<{ language: SupportedLanguage; file: string; source: string }> = [
        { language: 'typescript', file: 'sample.ts', source: 'export function add(a: number, b: number): number { return a + b; }' },
    ...
    for (const { language, file, source } of samples) {
        try {
            ...
            rows.push({ language, adapter: 'tree-sitter universal', status, symbols: r.symbols.length, relations: r.relations.length, note });
        } catch (error) {
            rows.push({ language, adapter: 'tree-sitter universal', status: 'fail', symbols: 0, relations: 0, note: error instanceof Error ? error.message : String(error) });
        }
    }
...
async function mcpSmoke(): Promise<{ ok: boolean; elapsed_ms: number; tools: number; stderr_tail: string }> {
    ...
    return await new Promise((resolve) => {
        ...
        const finish = (ok: boolean, tools = 0) => {
            ...
            settled = true;
            child.kill('SIGTERM');
            resolve({ ok, elapsed_ms: elapsedMs(start), tools, stderr_tail: stderr.trim().split(/\r?\n/).slice(-5).join('\n') });
        };
        ...
        child.on('exit', () => { if (!settled) finish(false); });
        child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'initialize', params: { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'multi-language-benchmark', version: '1.0.0' } } })}\n`);
...
function summarizeTokenRows(rows: TokenTask[], corpusTokens: number): {
    tasks: number;
    exact_tokens: number;
...
async function writeReport(smokeRows: SmokeRow[], results: TargetResult[], mcp: Awaited<ReturnType<typeof mcpSmoke>>): Promise<void> {
    ...
    lines.push('');
    lines.push(markdownTable(
        ['Repo', 'Primary Language', 'Source Tokens', 'Ingest Time', 'Files Indexed', 'Files Failed', 'Symbol Versions', 'Relations', 'Token Tasks', 'Full-Source Reduction', 'Exact-Symbol Reduction', 'Exact Savings'],
        results.map(result => {
    ...
    for (const result of results) {
        ...
        lines.push(markdownTable(
            ...
            Object.entries(result.corpus.by_extension).slice(0, 12).map(([ext, stats]) => [
                ext,
        ...
        lines.push('');
        lines.push(markdownTable(
            ['Symbol', 'File', 'Full Source Tokens', 'Exact Files', 'Exact Tokens', 'ContextZero Tokens', 'Full Reduction', 'Exact Reduction', 'Exact Savings', 'Exact Time', 'Context Time'],
    ...
    lines.push('## Caveats');
    lines.push('');
    lines.push('- These are local Windows measurements from this machine; ingestion timings include local disk, CPU, Node, and PostgreSQL pool behavior.');
    lines.push('- Token counts are byte-based estimates for repeatability, not exact model tokenizer counts.');
    lines.push('- Some repositories are intentionally multi-language; the primary language column names the benchmark focus, while the indexed-language table shows what actually landed in the graph.');
    lines.push('- PHP and Bash adapter smoke tests currently return zero symbols on this install because their tree-sitter grammar setup fails or yields no symbol nodes; Laravel and nvm are included to show that failure at repo level instead of hiding it.');
    lines.push('- The whole-source baseline represents an agent reading all source/document files for each task. The exact-symbol baseline is stricter and usually much smaller.');
    lines.push('');
...
async function main(): Promise<void> {
    ...
    const selected = process.env.BENCH_REPOS
        ? allTargets.filter(target => process.env.BENCH_REPOS!.split(',').map(s => s.trim()).includes(target.id))
        : allTargets;
    ...
    for (const target of selected) {
        ...
        if (cached) {
            ...
            results.push(cached);
            await writeReport(smokeRows, results, { ok: false, elapsed_ms: 0, tools: 0, stderr_tail: '[pending until final step]' });
            continue;
        }
