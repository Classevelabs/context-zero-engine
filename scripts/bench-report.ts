...
const skipDirs = new Set([
    '.git', 'node_modules', 'dist', 'coverage', '.next', '.turbo',
    '.cache', '.pytest_cache', '__pycache__',
]);

const sourceExts = new Set([
    '.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs', '.py', '.rs', '.go',
    '.java', '.cs', '.cpp', '.cc', '.cxx', '.h', '.hpp', '.rb', '.kt',
    '.swift', '.php', '.sh', '.sql', '.json', '.md', '.yml', '.yaml',
]);
...
async function collectIndexCounts(snapshotId: string, repoId: string): Promise<IndexCounts> {
    return {
        ...
        dispatch_edges: await countRows('SELECT COUNT(*)::int AS count FROM dispatch_edges WHERE snapshot_id = $1', [snapshotId]),
        class_hierarchy: await countRows('SELECT COUNT(*)::int AS count FROM class_hierarchy WHERE snapshot_id = $1', [snapshotId]),
        concept_families: await countRows('SELECT COUNT(*)::int AS count FROM concept_families WHERE snapshot_id = $1', [snapshotId]),
        ...
        temporal_co_changes: await countRows('SELECT COUNT(*)::int AS count FROM temporal_co_changes WHERE repo_id = $1', [repoId]),
        temporal_risk_scores: await countRows('SELECT COUNT(*)::int AS count FROM temporal_risk_scores WHERE snapshot_id = $1', [snapshotId]),
        runtime_traces: await countRows('SELECT COUNT(*)::int AS count FROM runtime_traces WHERE snapshot_id = $1', [snapshotId]),
        runtime_observed_edges: await countRows('SELECT COUNT(*)::int AS count FROM runtime_observed_edges WHERE snapshot_id = $1', [snapshotId]),
    };
}
...
async function selectTargets(snapshotId: string): Promise<Array<{
...
async function tokenSavingsBenchmarks(snapshotId: string, corpusForTokenBench: CorpusStats): Promise<TokenTask[]> {
    ...
    for (const target of targets) {
        ...
        rows.push({
            ...
            full_repo_savings_percent: round((1 - (contextzeroTokens / Math.max(1, corpusForTokenBench.source_estimated_tokens))) * 100, 2),
...
function collectBenchmarkCaveats(data: {
    ...
    if (python3.status !== 0) {
        caveats.push('`python3` is not available on this Windows PATH. Python extraction can be incomplete until Python is installed or the `python3` command is mapped to the installed Python runtime.');
    }
    if (filesFailed > 0 && python3.status !== 0) {
        caveats.push(`${formatNumber(filesFailed)} file(s) failed during ingestion on this Windows host, matching the missing \`python3\` runtime path for Python extraction. TypeScript/JavaScript indexing still completed.`);
    } else if (filesFailed > 0) {
        caveats.push(`${formatNumber(filesFailed)} file(s) failed during ingestion. Treat this as a partial-language benchmark unless those failures are resolved.`);
    }
    if (data.commands.some(command => command.stderr_tail.includes('Failed to recover stale transaction'))) {
        caveats.push('The test suite prints an error log from a transaction recovery test, but Jest still reports all suites and tests passing.');
    }
    if (lineagesComputed === 0) {
        caveats.push('Symbol lineage count was 0 in the ingestor result, so lineage-dependent benchmark dimensions should be treated as unavailable for this run.');
    }
    caveats.push('Ingestion wall time includes local PostgreSQL pool behavior and local disk/CPU limits on this Windows host.');
    if ((data.corpus.by_extension['.js']?.files || 0) > 0 || (data.corpus.by_extension['.mjs']?.files || 0) > 0) {
        caveats.push('This repo contains JavaScript/MJS utility and fixture files. The TypeScript program extractor can warn when those files are outside `tsconfig.json`; they are still included in corpus totals.');
    }
    if ((data.corpus.by_extension['.sh']?.files || 0) > 0) {
        caveats.push('This repo contains shell scripts. Native filesystem query helpers may emit tree-sitter warnings for some shell fixtures while still completing the timed query.');
    }
    if (data.mcp.stderr_tail) {
        caveats.push('The MCP smoke-test stderr tail is captured in the MCP section, including runtime warnings from dependencies.');
    }
...
async function writeReport(data: {
    ...
    lines.push(`Token estimate policy: \`ceil(bytes / ${tokenDivisor})\`. This is a deterministic approximation, not a provider tokenizer.`);
    lines.push('');
    ...
    lines.push('4. Query latency and output size: native overview, native symbol search, native code search, health, blast radius.');
    lines.push('5. Token savings: whole source/document repo read and exact-symbol grep-and-read vs ContextZero strict context capsule for real symbols.');
    ...
    lines.push(markdownTable(
        ...
        [
            ...
            ['MCP smoke test', data.mcp.ok ? `passed, ${data.mcp.tools} tools, ${data.mcp.elapsed_ms} ms` : `failed, ${data.mcp.elapsed_ms} ms`],
        ],
    ));
    lines.push('');
    ...
    lines.push('');
    lines.push('Whole-source baseline means: read every source/document file in the target repository. Exact-symbol baseline means: find every source/document file containing the exact symbol name and read those files into context. ContextZero means: compile a strict context capsule for the same symbol.');
    lines.push('');
    lines.push(markdownTable(
        ['Symbol', 'File', 'Full Source Tokens', 'Full Source Reduction', 'Exact Files', 'Exact Tokens', 'ContextZero Tokens', 'Exact Reduction', 'Exact Savings', 'Exact Time', 'ContextZero Time'],
    ...
    lines.push('');
    lines.push(markdownTable(
        ['Full Source Tokens', 'Exact Files', 'Exact Tokens', 'ContextZero Tokens', 'Full Source Reduction', 'Full Source Savings', 'Exact Reduction', 'Exact Savings', 'Exact Time', 'ContextZero Time'],
        [[
    ...
    lines.push('');
    lines.push('- These are local Windows measurements from this machine, so speed numbers include local disk, PostgreSQL, Node, and CPU behavior.');
    lines.push('- Token counts are deterministic byte-based estimates for repeatability. Exact Claude/OpenAI tokenizer counts will differ by model.');
    lines.push('- The token-saving comparison intentionally uses a simple, reproducible traditional baseline. A human or agent could read fewer files with more judgment, or far more files when tracing dependencies manually.');
    lines.push('- The current remote HTTP server is REST, not Streamable HTTP MCP. Local Codex/Claude integration is via stdio MCP.');
    lines.push('');
...
async function main(): Promise<void> {
    ...
    if (firstTarget) {
        queryMetrics.push(await timedQuery('blast_radius_depth_2', `Blast radius for ${firstTarget.canonical_name}.`, () =>
            blastRadiusEngine.computeBlastRadius(ingest.snapshot_id, [firstTarget.symbol_version_id], 2),
        ));
        queryMetrics.push(await timedQuery('capsule_strict', `Strict context capsule for ${firstTarget.canonical_name}.`, () =>
