  ...
  const res = await db.query(
    `
        ...
        WHERE f.snapshot_id = $1
          ...
          AND LENGTH(COALESCE(sb.body_source, '')) > 500
...
async function benchOne(
  target: { symbol_version_id: string; name: string; file_path: string },
  snapshotId: string,
): Promise<RowResult> {
  // ── Traditional path ──
  const t0 = Date.now()
  ...
  try {
    ...
    try {
      const radius = await blastRadiusEngine.computeBlastRadius(snapshotId, [target.symbol_version_id], 3)
...
async function main() {
  ...
  if (snap.rowCount === 0) {
    console.error(`no snapshot matching "${repoBase}%"`)
    process.exit(1)
  }
